...
- **Niveles de activación** (`activacion.ts`): máquina `SIMULADO→SANDBOX→PILOTO→REAL` (un paso adelante; retroceso a SIMULADO siempre = kill-switch). **Event-sourced en `RegistroAdaptador`** (`nivelActivacion`, nace `SIMULADO`; evento `nivel_cambiado`; servicio `cambiarNivel` con acto humano y transición válida). **SSOT del nivel = el registro**; el orquestador lo lee de ahí (`opciones.nivelActivacion` puede forzarlo en tests). REAL exige nivel que lo permita (PILOTO/REAL); si no → gate `ACTIVACION`. Kill-switch = `cambiarNivel(SIMULADO)`. El alcance concreto del PILOTO es D-5 (inyectado).
- **Registro de consumo post-ejecución** (`consumo.ts` + orquestador): tras una ejecución REAL exitosa con presupuesto y ledger, se contabiliza el **consumo estimado** (cota conservadora, no lo reportado por el adaptador — decisión de seguridad: un adaptador no puede sub-declarar su consumo). Cierra el loop del presupuesto.
- **Evidencia operativa v4**: añade `nivelActivacion` a `EvidenciaOperativa` (auditoría de la activación por ejecución).
