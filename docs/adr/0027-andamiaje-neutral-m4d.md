...
- **Niveles de activación** (`activacion.ts`): máquina `SIMULADO→SANDBOX→PILOTO→REAL` (un paso adelante; retroceso a SIMULADO siempre = kill-switch); `nivelPermiteReal`/`nivelPermiteModo`. El alcance del PILOTO es D-5 (inyectado); la máquina es neutral.
