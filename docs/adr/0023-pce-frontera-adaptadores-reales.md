...
- **M4-C-A** (este): contratos neutrales + estado de frontera + errores normalizados + sandbox + fake/grabado + evidencia + smoke opt-in determinista.
- **M4-C-B** (siguiente, a ratificar): capa opt-in de timer real para timeout; registro/gobernanza de adaptadores event-sourced ligado a la capacidad; revocación/expiración/eliminación (F-4 de ADR-0022); primer adaptador real DESACTIVADO con revisión específica de no-filtración.
