...
- **M4A-1 (ALTO) — referencia opaca real (Art. 4):** `secretRef` debe ser una referencia de una **allowlist CERRADA y versionada** de esquemas (`env/vault/aws-sm/gcp-sm/azure-kv/secretstore/file/ref`), con **path no vacío**, **y** no puede tener forma de secreto. La detección analiza el valor por **segmentos** (separados por `: / . _ -`): rechaza `sk-…`, `AKIA…`, `Bearer …`, valores con `=`/espacios, **prefijos de token** (`token…/apikey…/bearer…`) y **segmentos compactos alfanuméricos ≥20 con letras y dígitos** (R-1: cubre tokens de longitud media camuflados tras un esquema válido, p. ej. `vault:tokenABCDEF1234567890abcdef`), mientras permite referencias estructuradas (`vault:org-a/generation/main`). `proveedorRef` se valida como identificador lógico acotado. Guardarraíl **preventivo**: no es un detector criptográfico, no sustituye al SecretStore (M4-B) y nunca autoriza almacenar un secreto; el dominio sólo recibe referencias lógicas (`domain/referencias.ts`). R-2: `secretstore:` incorporado a la allowlist (usado en la Directiva).
