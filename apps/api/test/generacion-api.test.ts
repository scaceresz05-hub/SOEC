...
const attr: Attribution = { source: 'seed', purpose: 'test', assumptions: [], claimType: 'observational', regime: 'empirical', uncertainty: 'na' };
...
async function sembrarConocimiento(store: EventStore, slug: string): Promise<void> {
  ...
  const c: RequestContext = { organizationId: OrganizationId(slug), actor: ActorId('seed'), scope: { organizationId: OrganizationId(slug), permissions: ['events:append', 'events:read'] }, correlationId: 'seed' };
...
const PARAMS = { objetivoComercial: 'crecer', objetivoMarketing: 'generar leads', indicador: 'leads', valorEsperado: 100, horizonteDias: 30, prioridad: 'alta', presupuestoTotal: 100000, frecuenciaDias: 2, canales: ['correo'] };
...
async function montar(rateLimit?: RateLimiter) {
  ...
  const app = buildApp({ store, intelligence: new DeterministicIntelligenceProvider(), pool, legacyDemoAccess: false, ...(rateLimit ? { generationRateLimit: rateLimit } : {}) });
  ...
  const created = await app.inject({ method: 'POST', url: '/organizations', headers: { ...H, cookie }, payload: { slug: 'gen-org', name: 'Gen Org' } });
...
describe('Motor de Generación · API autenticada (Tramo J)', () => {
  it('sin sesión → 401 (la ausencia de sesión nunca autoriza)', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/generation/programas/prog1', headers: { ...H, 'x-organization-slug': slug } });
  ...
  it('con sesión pero sin membresía en la organización pedida → 404 (la URL/el header no autorizan)', async () => {
    ...
    const res = await app.inject({ method: 'GET', url: '/generation/programas/prog1', headers: auth(cookie, 'otra-org-inexistente') });
  ...
  it('flujo completo SIMULADO: start (201) → gate 409 → aprobar piezas → run-simulated (201, EVALUADO)', async () => {
    ...
    const start = await app.inject({ method: 'POST', url: '/generation/programas/prog1/start', headers: auth(cookie, slug), payload: PARAMS });
    ...
    for (const path of ['creative-strategies', 'campaigns', 'content', 'experiments', 'calendar', 'approvals']) {
      const r = await app.inject({ method: 'GET', url: `/generation/programas/prog1/${path}`, headers: auth(cookie, slug) });
    ...
    const pendiente = await app.inject({ method: 'POST', url: '/generation/programas/prog1/run-simulated', headers: auth(cookie, slug), payload: {} });
    ...
    const run = await app.inject({ method: 'POST', url: '/generation/programas/prog1/run-simulated', headers: auth(cookie, slug), payload: { modo: 'PILOT' } });
  ...
  it('rechaza la ejecución en modo real (AUTONOMOUS_REAL) → 422 MODO_REAL_BLOQUEADO', async () => {
    const { app, cookie, slug } = await montar();
    await app.inject({ method: 'POST', url: '/generation/programas/prog1/start', headers: auth(cookie, slug), payload: PARAMS });
    const real = await app.inject({ method: 'POST', url: '/generation/programas/prog1/run-simulated', headers: auth(cookie, slug), payload: { real: true } });
  ...
  it('autorización fina: un VIEWER no puede iniciar generación → 403 (permiso atómico faltante)', async () => {
    const { app, cookie, slug } = await montar();
    const inv = await app.inject({ method: 'POST', url: `/organizations/${slug}/invitations`, headers: { ...H, cookie }, payload: { email: 'viewer@x.com', role: 'VIEWER' } });
    const token = inv.json().devToken as string;
    const acc = await app.inject({ method: 'POST', url: `/invitations/${token}/accept`, headers: H, payload: { displayName: 'Viewer', password: 'Password123' } });
    const cookieViewer = cookieDe(acc);
    // El VIEWER es miembro (no es 404) pero carece de generation.start → 403.
    const res = await app.inject({ method: 'POST', url: '/generation/programas/prog1/start', headers: auth(cookieViewer, slug), payload: PARAMS });
    expect(res.statusCode).toBe(403);
    // …aunque SÍ puede leer (generation.read es transversal).
    const lectura = await app.inject({ method: 'GET', url: '/generation/programas/prog1', headers: auth(cookieViewer, slug) });
  ...
  it('rate limiting de acciones de generación: supera el límite → 429 con retry-after', async () => {
    const { app, cookie, slug } = await montar(new RateLimiter({ maxIntentos: 2, ventanaMs: 60_000, bloqueoMs: 60_000 }));
    const call = () => app.inject({ method: 'POST', url: '/generation/programas/prog1/start', headers: auth(cookie, slug), payload: PARAMS });
