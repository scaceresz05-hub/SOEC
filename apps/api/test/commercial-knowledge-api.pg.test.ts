...
async function montar() {
  ...
  const created = await app.inject({
    ...
    url: '/organizations',
    headers: { ...H, cookie },
    payload: { slug: 'ck-org', name: 'CK Org' },
  });
...
async function poblar(
  app: Awaited<ReturnType<typeof montar>>['app'],
  cookie: string,
  slug: string,
) {
  ...
  await app.inject({
    ...
    url: `${ck}/entities`,
    headers: h,
    payload: { id: 'empresa', tipo: 'EMPRESA', nombre: 'SmileFlow' },
  });
  await app.inject({
    method: 'PATCH',
    url: `${ck}/entities/empresa`,
    headers: h,
    payload: { clave: 'propuestaValor', valor: 'Odontología cercana y a plazos' },
  });
  await app.inject({
    method: 'POST',
    url: `${ck}/entities`,
    headers: h,
    payload: { id: 'p1', tipo: 'PRODUCTO', nombre: 'Ortodoncia invisible' },
  });
  await app.inject({
    method: 'PATCH',
    url: `${ck}/entities/p1`,
    headers: h,
    payload: { clave: 'problemaQueResuelve', valor: 'alinear dientes sin brackets' },
  });
  await app.inject({
    method: 'PATCH',
    url: `${ck}/entities/p1`,
    headers: h,
    payload: { clave: 'beneficios', valor: 'sonrisa alineada discreta' },
  });
  await app.inject({
    method: 'POST',
    url: `${ck}/entities`,
    headers: h,
    payload: { id: 'icp1', tipo: 'CLIENTE_IDEAL', nombre: 'Adultos jóvenes' },
  });
  await app.inject({
    method: 'PATCH',
    url: `${ck}/entities/icp1`,
    headers: h,
    payload: { clave: 'dolores', valor: 'vergüenza por dientes torcidos' },
  });
  await app.inject({
    method: 'POST',
    url: `${ck}/hypotheses`,
    ...
    payload: {
      id: 'h1',
      enunciado: 'Correo convierte para el ICP joven',
      contexto: 'canales',
      segmentoId: 'icp1',
    },
  });
  await app.inject({
    method: 'POST',
    url: `${ck}/hypotheses/h1/evidence`,
    headers: h,
    payload: { descripcion: 'ICP responde a email', origen: 'DATO_IMPORTADO', aFavor: true },
  });
}
...
describe('Conocimiento comercial · API autenticada (A-1)', () => {
  ...
  it('un VIEWER no puede escribir conocimiento → 403; sí puede leer', async () => {
    ...
    const inv = await app.inject({
      ...
      headers: { ...H, cookie },
      payload: { email: 'v@x.com', role: 'VIEWER' },
    });
    const acc = await app.inject({
      method: 'POST',
      url: `/invitations/${inv.json().devToken}/accept`,
      headers: H,
      payload: { displayName: 'V', password: 'Password123' },
    });
    ...
    expect(
      (
        await app.inject({
          method: 'POST',
          url: '/commercial-knowledge/entities',
          headers: auth(cv, slug),
          payload: { tipo: 'EMPRESA', nombre: 'X' },
        })
      ).statusCode,
    ).toBe(403);
    expect(
      (await app.inject({ method: 'GET', url: '/commercial-knowledge', headers: auth(cv, slug) }))
        .statusCode,
    ).toBe(200);
  });
  ...
  it('poblar el CRM por API persiste y deja el motor LISTO; start ya no ABSTIENE', async () => {
    ...
    const cov0 = await app.inject({
      method: 'GET',
      url: '/commercial-knowledge/coverage',
      headers: auth(cookie, slug),
    });
    expect(cov0.json().listoParaGenerar).toBe(false);
    const start0 = await app.inject({
      method: 'POST',
      url: '/generation/programas/progA/start',
      headers: auth(cookie, slug),
      payload: { objetivoComercial: 'crecer', objetivoMarketing: 'leads', canales: ['correo'] },
    });
    ...
    const hips = await app.inject({
      method: 'GET',
      url: '/commercial-knowledge/hypotheses',
      headers: auth(cookie, slug),
    });
  ...
  it('asociar una hipótesis a un ICP inexistente → 400 gobernado', async () => {
    ...
    await app.inject({
      ...
      url: '/commercial-knowledge/hypotheses',
      headers: auth(cookie, slug),
      payload: { id: 'h9', enunciado: 'algo' },
    });
    const res = await app.inject({
      method: 'POST',
      url: '/commercial-knowledge/hypotheses/h9/segment',
      headers: auth(cookie, slug),
      payload: { segmentoId: 'no-existe' },
    });
