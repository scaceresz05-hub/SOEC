...
import { makePool, runMigrations, migrations, PgEventStore } from '@soec/event-store/pg';
...
const H = { 'x-organization-id': 'org-api', 'x-actor-id': 'director', 'x-scope': 'events:append,events:read' };
...
beforeAll(async () => { await runMigrations(pool, migrations); });
afterAll(async () => { await pool.end(); });
beforeEach(async () => { await pool.query('truncate table events restart identity cascade'); });
...
describe('apps/api · CIA sobre PostgreSQL (HTTP + reinicio)', () => {
  it('autoriza y planifica por HTTP; el estado persiste y se lee tras reiniciar', async () => {
    const a1 = app();
    const auth = await a1.inject({ method: 'POST', url: '/api/cia/autorizaciones/captar-clientes-publicidad', headers: H, payload: { limite: 100000, nivelAutonomia: 'EJECUTAR_AUTOMATICO', actorHumano: 'humano-1' } });
    expect(auth.statusCode).toBe(200);
    expect(auth.json().datos.estado).toBe('AUTORIZADA');

    const plan = await a1.inject({ method: 'POST', url: '/api/cia/planes', headers: H, payload: { planId: 'pa', capacidadId: 'captar-clientes-publicidad', objetivo: 'pacientes', costoEstimado: 4000 } });
    ...
    const capacidades = inicio.json().datos.capacidades as Array<{ capacidadId: string; estado: string }>;
    expect(capacidades.some((c) => c.capacidadId === 'captar-clientes-publicidad' && c.estado === 'Activa')).toBe(true);

    // la vista de usuario (explicación) no filtra proveedor; la auditoría sí
    const exp = await a2.inject({ method: 'GET', url: '/api/cia/planes/pa/explicacion', headers: H });
    expect(JSON.stringify(exp.json().datos)).not.toContain('ads-');
    const audit = await a2.inject({ method: 'GET', url: '/api/cia/planes/pa/auditoria', headers: H });
  ...
  it('el catálogo HTTP no expone proveedores comerciales', async () => {
    ...
    for (const marca of ['meta', 'google', 'mailchimp', 'hubspot', 'tiktok']) expect(serial.includes(marca)).toBe(false);
    await a.close();
  });
  ...
  it('el kill-switch por HTTP frena una nueva planificación', async () => {
    const a = app();
    await a.inject({ method: 'POST', url: '/api/cia/autorizaciones/dar-a-conocer-marca', headers: H, payload: { limite: 100000, nivelAutonomia: 'EJECUTAR_AUTOMATICO', actorHumano: 'humano-1' } });
    await a.inject({ method: 'POST', url: '/api/cia/autonomia/kill/ORG', headers: H });
    const plan = await a.inject({ method: 'POST', url: '/api/cia/planes', headers: H, payload: { planId: 'pk', capacidadId: 'dar-a-conocer-marca', objetivo: 'x', costoEstimado: 10 } });
