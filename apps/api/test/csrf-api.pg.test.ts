...
describe('API · protección CSRF/Origin (F-01)', () => {
  it('POST con Origin permitido + cookie → permitido; con Origin ajeno → 403', async () => {
    ...
    const ok = await app.inject({
      method: 'POST',
      url: '/organizations',
      headers: { ...H, cookie, origin: ORIGEN_OK },
      payload: { slug: 'org-ok', name: 'OK' },
    });
    ...
    const bad = await app.inject({
      method: 'POST',
      url: '/organizations',
      headers: { ...H, cookie, origin: ORIGEN_AJENO },
      payload: { slug: 'org-evil', name: 'X' },
    });
  ...
  it('PATCH y DELETE con Origin ajeno → 403', async () => {
    ...
    await app.inject({
      ...
      url: '/auth/register',
      headers: H,
      payload: { email: 'o@x.com', displayName: 'O', password: 'Password123' },
    });
    const login = await app.inject({
      method: 'POST',
      url: '/auth/login',
      headers: H,
      payload: { email: 'o@x.com', password: 'Password123' },
    });
    const cookie = cookieDe(login);
    await app.inject({
      method: 'POST',
      url: '/organizations',
      headers: { ...H, cookie, origin: ORIGEN_OK },
      payload: { slug: 'org-a', name: 'A' },
    });

    const patch = await app.inject({
      method: 'PATCH',
      url: '/organizations/org-a',
      headers: { ...H, cookie, origin: ORIGEN_AJENO },
      payload: { name: 'hack' },
    });
    ...
    const del = await app.inject({
      method: 'DELETE',
      url: '/organizations/org-a/members/algo',
      headers: { ...H, cookie, origin: ORIGEN_AJENO },
    });
