...
async function registrarYLoguear(app: App, email: string): Promise<string> {
  await app.inject({
    ...
    url: '/auth/register',
    headers: H,
    payload: { email, displayName: 'U', password: 'Password123' },
  });
...
describe('API · autenticación (sin sesión ⇒ 401)', () => {
    ...
    expect(
      (
        await app.inject({
          method: 'GET',
          url: '/organizations',
          headers: { cookie: 'soec_session=inexistente' },
        })
      ).statusCode,
    ).toBe(401);
  });
  ...
  it('login incorrecto → 401 genérico (no enumera)', async () => {
    ...
    await app.inject({
      ...
      url: '/auth/register',
      headers: H,
      payload: { email: 'a@x.com', displayName: 'A', password: 'Password123' },
    });
    expect(
      (
        await app.inject({
          ...
          url: '/auth/login',
          headers: H,
          payload: { email: 'a@x.com', password: 'mala' },
        })
      ).statusCode,
    ).toBe(401);
    expect(
      (
        await app.inject({
          ...
          url: '/auth/login',
          headers: H,
          payload: { email: 'noexiste@x.com', password: 'Password123' },
        })
      ).statusCode,
    ).toBe(401);
  });
...
describe('API · aislamiento multi-tenant', () => {
  it('7-8-13. usuario de A no ve ni modifica B; :org manipulado no da autoridad (404)', async () => {
    ...
    expect(
      (await app.inject({ method: 'GET', url: '/organizations/org-a', headers: { cookie: a } }))
        .statusCode,
    ).toBe(200);
    expect(
      (await app.inject({ method: 'GET', url: '/organizations/org-b', headers: { cookie: a } }))
        .statusCode,
    ).toBe(404);
  ...
  it('9-10. VIEWER no administra miembros (403); OPERATOR no cambia modo (403)', async () => {
    ...
    const invV = await app.inject({
      method: 'POST',
      url: '/organizations/org-a/invitations',
      headers: { ...H, cookie: owner },
      payload: { email: 'viewer@x.com', role: 'VIEWER' },
    });
    const tokenV = invV.json().devToken as string;
    const accV = await app.inject({
      method: 'POST',
      url: `/invitations/${tokenV}/accept`,
      headers: H,
    ...
    expect(
      (
        await app.inject({
          method: 'POST',
          url: '/organizations/org-a/invitations',
          headers: { ...H, cookie: cookieV },
          payload: { email: 'x@x.com', role: 'ANALYST' },
        })
      ).statusCode,
    ).toBe(403);
    // OPERATOR intenta cambiar modo → 403.
    const invO = await app.inject({
      method: 'POST',
      url: '/organizations/org-a/invitations',
      headers: { ...H, cookie: owner },
      payload: { email: 'op@x.com', role: 'MARKETING_OPERATOR' },
    });
    const accO = await app.inject({
      method: 'POST',
      url: `/invitations/${invO.json().devToken}/accept`,
      headers: H,
      payload: { displayName: 'O', password: 'Password123' },
    });
    expect(
      (
        await app.inject({
          method: 'PATCH',
          url: '/organizations/org-a/operational-mode',
          headers: { ...H, cookie: cookieDe(accO) },
          payload: { mode: 'SUPERVISED_REAL' },
        })
      ).statusCode,
    ).toBe(403);
  });
...
describe('API · cutover de la superficie vertical (18)', () => {
  it('sin sesión, una ruta vertical real → 401: ya NO es alcanzable sin autenticación', async () => {
    ...
    const res = await app.inject({
      method: 'GET',
      url: '/experiencia/comprender-estado/historial',
    });
  ...
  it('con sesión pero sin organización → 400; con sesión + organización propia el gateway deja pasar', async () => {
    ...
    const ok = await app.inject({
      method: 'GET',
      url: '/experiencia/comprender-estado/historial',
      headers: { cookie, 'x-organization-slug': 'org-a' },
    });
  ...
  it('con sesión, indicar una organización ajena NO da autoridad (404)', async () => {
    ...
    const res = await app.inject({
      method: 'GET',
      url: '/experiencia/comprender-estado/historial',
      headers: { cookie: a, 'x-organization-slug': 'org-b' },
    });
