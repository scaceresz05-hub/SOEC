...
function filas(externalRef: string, escenario: Escenario, seq = 1): FilaProveedor[] {
  const f = (metrica: string, valor: number, unidad = 'conteo', moneda: string | null = null): FilaProveedor => ({ externalId: externalRef, metrica, valor, unidad, moneda, periodo: '2026-07-21', ocurridoEn: '2026-07-21T00:00:00.000Z', proveedorSeq: seq, acumulativa: true, estimada: false });
  ...
  return [f('impresiones', 1000), f('clics', 100), f('leads', 40), f('conversiones', 8), f('gasto', 200, 'monetario', 'CLP')];
}
...
export class MeasurementExperience {
  ...
  constructor(private readonly store: EventStore) {
    ...
    creds.registrarTodosLosCanales(ORG, CUENTA, CRED, ['blog', 'linkedin', 'correo', 'instagram', 'meta_ads', 'facebook']);
    const simulado = new AdaptadorCanalSimulado();
    this.publicaciones = new PublicationService(store, { simulado, sandbox: simulado }, creds, this.content);
  ...
  private ctx(): RequestContext {
    ...
    return { organizationId, actor: ActorId('soec'), scope: { organizationId, permissions: ['events:append', 'events:read'] }, correlationId: `exp-met-${ORG}` };
  }
  private now(): string {
  ...
  async preparar(): Promise<void> {
    ...
    for (const canal of PUBLICABLES) {
      ...
      await this.content.prepararContenidoParaActividad(ctx, { planId: IDS_MKT_CONT.plan, actividadId, marcaId: IDS_CONT.marca, promptPiezaId: IDS_CONT.promptPieza, promptAdaptId: IDS_CONT.promptAdapt, ganchosPromocionales: CONT_GANCHOS, attribution: ATRIBUCION, occurredAt: this.now() });
      const pub = await this.publicaciones.cargar(ctx, this.pubId(actividadId, canal));
      if (!pub.existe) {
        await this.publicaciones.publicarCiclo(ctx, { paqueteId: this.paqueteId(actividadId), canal, policyId: IDS_MKT_CONT.politica, modo: 'simulado', cuentaLogica: CUENTA, credencialId: CRED, attribution: ATRIBUCION, occurredAt: this.now() });
      }
    }
  }
  ...
  async sincronizarTodo(escenario: Escenario): Promise<{ medidas: number }> {
    ...
    for (const canal of PUBLICABLES) {
      ...
      await this.medicionSvc.sincronizar(ctx, { publicationId: pub.publicationId, externalRef: pub.externalRef, canal, cuenta: CUENTA, token: 't', campaniaRef: `cmp-${canal}`, objetivoRef: IDS_MKT_CONT.objetivo, criterio: CRITERIO_DEMO, gastoAutorizado: GASTO_AUTORIZADO_DEMO, muestraMinima: 500, attribution: ATRIBUCION, occurredAt: this.now() });
      medidas += 1;
    }
    return { medidas };
  }
  ...
  async optimizarTodo(): Promise<{ propuestas: number; aplicadas: number; denegadas: number }> {
    ...
    for (const canal of PUBLICABLES) {
      ...
      const opt = await this.optimizacionSvc.optimizar(ctx, { publicationId: pub.publicationId, planId: IDS_MKT_CONT.plan, campaniaId: `cmp-${canal}`, actividadId, canal, objetivoId: IDS_MKT_CONT.objetivo, policyIdOperacional: IDS_MKT_CONT.politica, policyOpt: POLICY_OPT_DEMO, attribution: ATRIBUCION, occurredAt: this.now() });
      propuestas += 1;
  ...
  private resumenOpt(o: OptimizacionState): { tipo: string; estado: string; motivoDenegacion: string | null } {
    return { tipo: o.decision?.tipo ?? '—', estado: o.estado, motivoDenegacion: o.motivoDenegacion };
  }
  ...
  async estado(): Promise<EstadoMedicion> {
    ...
    for (const canal of PUBLICABLES) {
      ...
      const opt = pub.existe ? await this.optimizacionSvc.cargar(ctx, `${pub.publicationId}__opt${med?.sincronizaciones ?? 1}`) : null;
      actividades.push({
        id: actividadId,
        canal,
        ...
        atribucion: med?.atribucion ? { modelo: med.atribucion.modelo, clase: med.atribucion.clase, conversiones: med.atribucion.conversiones } : null,
        anomalias: (med?.anomalias ?? []).map((a) => ({ codigo: a.codigo, severidad: a.severidad })),
        optimizacion: opt && opt.existe ? this.resumenOpt(opt) : null,
      });
    }
    return { existe: plan.existe, empresa: objetivoContenidoDemo.empresa, escenario: this.escenario, actividades };
  }
}
