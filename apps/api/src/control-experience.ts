...
import {
  ContentService, MarcaService, PromptService, ProveedorGenerativoDeterminista,
  CONT_GANCHOS, IDS_CONT, IDS_MKT_CONT, marcaDemo, objetivoContenidoDemo, optsContenidoDemo, politicaContenidoDemo, promptAdaptDemo, promptPiezaDemo,
} from '@soec/contenido';
import { AdaptadorCanalSimulado, FixtureCredentialProvider, PublicationService } from '@soec/canales';
import {
  MeasurementService, OptimizationService, CRITERIO_DEMO, GASTO_AUTORIZADO_DEMO, POLICY_OPT_DEMO,
  FuenteMetricasSimulada, medStreamId, reconstruirMed, type FilaProveedor, type MedState,
} from '@soec/medicion';
import {
  PausaService, DecisionService, InboxService, calcularSalud,
  type Alcance, type ContenidoDecision, type EstadoSalud, type Rol, type SenalesSalud,
} from '@soec/control';

const ORG = 'pyme-ctrl-demo';
const CANALES = ['blog', 'linkedin', 'correo'];
const A: Attribution = { source: 'centro-de-control', purpose: 'gobernar el departamento autónomo', assumptions: ['datos sintéticos; sin gasto real; modo real desactivado'], claimType: 'observational', regime: 'institutional', uncertainty: 'baja' };
...
function filas(ref: string, esc: Escenario, seq = 1): FilaProveedor[] {
  ...
  if (esc === 'insuficiente') return [f('impresiones', 20), f('clics', 2), f('leads', 1), f('conversiones', 0), f('gasto', 5, 'monetario', 'CLP')];
...
export class ControlExperience {
  ...
  private ctx(): RequestContext {
    ...
    return { organizationId, actor: ActorId('soec'), scope: { organizationId, permissions: ['events:append', 'events:read'] }, correlationId: `exp-ctrl-${ORG}` };
  }
  private now(): string {
  ...
  async preparar(): Promise<void> {
    ...
    for (const canal of CANALES) {
      const a = `act-${canal}-0`;
      await this.content.prepararContenidoParaActividad(ctx, { planId: IDS_MKT_CONT.plan, actividadId: a, marcaId: IDS_CONT.marca, promptPiezaId: IDS_CONT.promptPieza, promptAdaptId: IDS_CONT.promptAdapt, ganchosPromocionales: CONT_GANCHOS, attribution: A, occurredAt: this.now() });
      if (!(await this.publicaciones.cargar(ctx, this.pubId(a, canal))).existe) {
        await this.publicaciones.publicarCiclo(ctx, { paqueteId: this.paqueteId(a), canal, policyId: IDS_MKT_CONT.politica, modo: 'simulado', cuentaLogica: 'cuenta-demo', credencialId: 'cred-demo', attribution: A, occurredAt: this.now() });
      }
    }
  }
  ...
  async simular(escenario: Escenario): Promise<{ medidas: number; aplicadas: number; decisiones: number; alertas: number; pausado: boolean }> {
    ...
    for (const canal of CANALES) {
      ...
      if (anom) {
        await this.inbox.registrarAlerta(ctx, { clave: `gasto:${pub.publicationId}`, tipo: 'gasto_anomalo', severidad: 'critico', origen: 'medicion', entidad: pub.publicationId, evidencia: anom.evidencia, impacto: anom.impacto, accionAutomatica: 'escalamiento bloqueado', accionHumana: 'reconciliar el gasto' }, A, this.now());
        alertas += 1;
      }
      const opt = await this.optimizacionSvc.optimizar(ctx, { publicationId: pub.publicationId, planId: IDS_MKT_CONT.plan, campaniaId: `cmp-${canal}`, actividadId: a, canal, objetivoId: IDS_MKT_CONT.objetivo, policyIdOperacional: IDS_MKT_CONT.politica, policyOpt: POLICY_OPT_DEMO, attribution: A, occurredAt: this.now() });
      ...
      if (opt.estado === 'denegada' && opt.decision?.tipo === 'aumentar_frecuencia') {
        const dec: ContenidoDecision = { tipo: 'escalamiento_frecuencia', razon: opt.decision.motivo, alcance: `${canal}/${a}`, efectoEsperado: 'aumentar la frecuencia de la actividad', riesgo: 'medio', presupuestoImplicado: 0, evidencia: opt.decision.evidencia, alternativas: ['mantener', 'esperar más datos'], recomendacionSistema: 'aprobar si el objetivo estratégico prioriza volumen', politica: IDS_MKT_CONT.politica, refPlan: IDS_MKT_CONT.plan };
  ...
  async pausar(tipo: Alcance['tipo'], valor: string, motivo: string, actor: string): Promise<{ pausaTotal: boolean }> {
    const s = await this.pausa.pausar(this.ctx(), { tipo, valor }, motivo, actor, A, this.now());
    return { pausaTotal: !!s.activas['departamento:*'] };
  }
  async reanudar(tipo: Alcance['tipo'], valor: string, actor: string): Promise<{ pausaTotal: boolean }> {
  ...
  async decisionesPendientes() {
    const ctx = this.ctx();
    const out: { decId: string; tipo: string; riesgo: string; razon: string; estado: string; recomendacion: string }[] = [];
    for (const canal of CANALES) {
      const d = await this.decisiones.cargar(ctx, `dec-${this.pubId(`act-${canal}-0`, canal)}`);
      if (d.existe && d.contenido) out.push({ decId: d.decId, tipo: d.contenido.tipo, riesgo: d.contenido.riesgo, razon: d.contenido.razon, estado: d.estado, recomendacion: d.contenido.recomendacionSistema });
    }
    return out;
  }

  async resolverDecision(decId: string, estado: 'aprobada' | 'denegada' | 'pospuesta', rol: Rol, actor: string, comentario: string): Promise<{ estado: string; efectoAplicado: boolean }> {
    const ctx = this.ctx();
    const d = await this.decisiones.resolver(ctx, decId, { estado, actor, rol, comentario }, A, this.now());
    ...
    if (estado === 'aprobada' && d.contenido?.tipo === 'escalamiento_frecuencia') {
      ...
      await this.planning.aplicarOptimizacion(ctx, IDS_MKT_CONT.plan, { tipo: 'aumentar_frecuencia', actividadId: a, valorAnterior: 'actual', valorNuevo: '+1/periodo', motivo: `aprobado por ${actor}`, optRef: decId, nuevoEstadoActividad: null }, A, this.now());
      efectoAplicado = true;
    }
    return { estado: d.estado, efectoAplicado };
  }

  private async senales(ctx: RequestContext): Promise<{ senales: SenalesSalud; meds: Record<string, MedState>; bloqueos: number }> {
    ...
    const pendientes = (await this.decisionesPendientes()).filter((d) => d.estado === 'pendiente').length;
    const bloqueos = Object.values(plan.actividades).filter((a) => a.estado === 'bloqueada' || a.estado === 'omitida').length;
    const alertasCriticas = Object.values(inbox.alertas).filter((al) => al.severidad === 'critico' && al.estado !== 'resuelta').length;
    const senales: SenalesSalud = { pausaTotal: !!pausa.activas['departamento:*'], riesgoCritico: riesgoCritico + alertasCriticas, intervencionRequerida: pendientes, bloqueos, advertencias, conDatos };
  ...
  async resumen() {
    ...
    for (const canal of CANALES) {
      ...
      if (med?.evaluacion) {
        objetivos.push({ objetivoId: `${IDS_MKT_CONT.objetivo}:${canal}`, indicador: med.evaluacion.indicador, lineaBase: med.evaluacion.lineaBase, meta: med.evaluacion.meta, resultado: med.evaluacion.resultado, calidad: med.calidad, clasificacion: med.evaluacion.clasificacion });
      }
    }
    optAplicadas = plan.optimizaciones.length;
    const excepciones = [];
    for (const al of Object.values(inbox.alertas).filter((x) => x.estado !== 'resuelta')) {
      excepciones.push({ tipo: al.tipo, severidad: al.severidad, modulo: al.origen, entidad: al.entidad, descripcion: al.evidencia, accionAutomatica: al.accionAutomatica, accionHumana: al.accionHumana, estado: al.estado });
    }
    ...
    return {
      ...
      proximos: Object.values(plan.actividades).filter((x) => x.estado === 'autorizable').slice(0, 5).map((x) => ({ actividadId: x.id, canal: x.canal, fecha: x.fechaProgramada, estadoContenido: x.paqueteContenidoRef ? 'listo' : 'pendiente', estado: x.estado })),
      excepciones,
      presupuesto: { moneda: 'CLP', produccion: CANALES.length * 3, distribucion: 0, publicidad: gastoTotal, planificado: politicaContenidoDemo.presupuestoTotal, comprometido: gastoTotal, ejecutado: gastoTotal, disponible: Math.max(0, politicaContenidoDemo.presupuestoTotal - gastoTotal), discrepancia: gastoTotal > politicaContenidoDemo.presupuestoTotal ? gastoTotal - politicaContenidoDemo.presupuestoTotal : 0 },
      decisionesPendientes: (await this.decisionesPendientes()).filter((d) => d.estado === 'pendiente').length,
  ...
  async actividad() {
    ...
    for (const o of plan.optimizaciones) out.push({ texto: `SOEC ${o.tipo === 'pausar_actividad' ? 'pausó' : 'ajustó'} la actividad ${o.actividadId}: ${o.motivo}. El plan quedó versionado.`, automatico: true, simulado: true });
    for (const canal of CANALES) {
      const pub = await this.publicaciones.cargar(ctx, this.pubId(`act-${canal}-0`, canal));
      if (pub.existe && pub.estado === 'verificada') out.push({ texto: `SOEC publicó y verificó la pieza de ${canal} (efecto simulado).`, automatico: true, simulado: true });
    }
    return out.reverse();
  }
  ...
  async alertas() {
    ...
    return Object.values(inbox.alertas).map((a) => ({ tipo: a.tipo, severidad: a.severidad, estado: a.estado, evidencia: a.evidencia, accionAutomatica: a.accionAutomatica }));
  }
  ...
  async auditoria(canal: string) {
    ...
    return {
      ...
      plan: { id: IDS_MKT_CONT.plan, version: plan.planVersion, optimizaciones: plan.optimizaciones.length },
      actividad: { id: a, estado: plan.actividades[a]?.estado ?? null, paquete: plan.actividades[a]?.paqueteContenidoRef ?? null },
      publicacion: pub.existe ? { estado: pub.estado, externalRef: pub.externalRef, modo: pub.modo, policyVersion: pub.policyVersion } : null,
      medicion: med?.evaluacion ? { clasificacion: med.evaluacion.clasificacion, calidad: med.calidad, atribucion: med.atribucion?.clase } : null,
    };
  }
}
