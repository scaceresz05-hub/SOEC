...
export function registerControlRoutes(app: FastifyInstance, store: EventStore): void {
  ...
  app.get('/control/actividad', async (_req, reply) =>
    reply.send({ entradas: await exp.actividad() }),
  );
  app.get('/control/decisiones', async (_req, reply) =>
    reply.send({ decisiones: await exp.decisionesPendientes() }),
  );
  app.get('/control/alertas', async (_req, reply) => reply.send({ alertas: await exp.alertas() }));
  app.get('/control/auditoria/:canal', async (req, reply) =>
    reply.send(await exp.auditoria((req.params as { canal: string }).canal)),
  );

  app.post('/control/simular', async (req, reply) => {
    const { escenario } = (req.body ?? {}) as {
      escenario?: 'bajo' | 'alto' | 'insuficiente' | 'gasto_excedido';
    };
    ...
    return reply
      ...
      .send(
        await exp.pausar(
          b.tipo ?? 'departamento',
          b.valor ?? '*',
          b.motivo ?? 'pausa del propietario',
          b.actor ?? 'propietario',
        ),
      );
  });
  app.post('/control/reanudar', async (req, reply) => {
    ...
    return reply
      .code(201)
      .send(await exp.reanudar(b.tipo ?? 'departamento', b.valor ?? '*', b.actor ?? 'propietario'));
  });
