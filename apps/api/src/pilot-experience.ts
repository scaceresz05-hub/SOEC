...
import {
  ...
  POLITICA_INICIAL_VERSION,
  entornoOperable,
  type EscenarioEnsayo,
  type Entorno,
  type Incidencia,
  type PasoEnsayo,
...
const A: Attribution = {
  source: 'experiencia-piloto',
  purpose: 'preparar un piloto operacional controlado',
  assumptions: ['organización sintética; sin efecto/gasto real; activación bloqueada'],
...
export class PilotExperience {
  ...
  async preparar(): Promise<void> {
    ...
    if (!org.existe) {
      ...
      await this.expSvc.crear(
        ctx,
        IDS_PILOTO.expediente,
        {
          orgRef: IDS_PILOTO.org,
          departamento: 'marketing',
          entorno: 'sandbox',
          objetivo: objetivoContenidoDemo.objetivoComercial,
          duracionDias: perfilDemo.duracionDias,
          criteriosExito: criteriosExitoDemo,
          criteriosSuspension: criteriosSuspensionDemo,
    ...
    if (!plan.existe) {
      ...
      await this.publicaciones.publicarCiclo(ctx, {
        paqueteId: `${IDS_MKT_CONT.plan}--act-blog-0`,
        canal: 'blog',
        policyId: IDS_MKT_CONT.politica,
        modo: 'simulado',
  ...
  async ensayar(
    ...
    if (previo.existe)
      return {
        ...
        pasos: previo.pasos.length,
        incidencias: previo.incidencias.length,
        rollbackVerificado: previo.rollbackVerificado,
      };
    ...
    if (escenario === 'presupuesto_invalido') {
      await this.orgSvc.definirPresupuesto(
        ctx,
        IDS_PILOTO.org,
        { ...presupuestoDemo, limiteDiario: 999, limiteTotal: 100 },
        A,
        this.now(),
      );
    }
    ...
    pasos.push({
      nombre: 'readiness',
      estado: ev.resultado === 'bloqueado' ? 'bloqueado' : 'ok',
      detalle: `readiness ${ev.resultado} (${entorno})`,
    });

    if (ev.resultado === 'bloqueado' || ev.resultado === 'incompleto') {
      resultado = 'bloqueado';
      const cb = ev.chequeos.find(
        (c) => c.bloqueo || c.estado === 'bloqueado' || c.estado === 'pendiente',
      );
      if (cb)
        incidencias.push({
          codigo: cb.codigo,
          ...
          severidad: 'mayor',
          descripcion: cb.requisito,
          evidencia: cb.evidencia,
          accionInmediata: cb.resolucion,
          estado: 'abierta',
        });
    ...
    if (escenario === 'activo_faltante') {
      pasos.push({
        ...
        estado: 'bloqueado',
        detalle:
          'canal visual real exige archivo real; en ensayo emulado el canal de texto sí publica',
      });
      resultado = 'bloqueado';
      incidencias.push({
        codigo: 'activo_real_faltante',
        categoria: 'contenido',
        severidad: 'mayor',
        descripcion: 'un canal visual requiere una imagen real',
        evidencia: 'solo hay especificación',
        accionInmediata: 'cargar el archivo real (bloqueado en este bloque)',
        estado: 'abierta',
      });
      return this.finalizar(
        ctx,
    ...
    pasos.push({
      nombre: 'publicacion_emulada',
      estado: pub.existe && pub.estado === 'verificada' ? 'ok' : 'omitido',
      detalle: `publicación ${pub.externalRef ?? '—'} (${pub.estado})`,
    });
    if (pub.existe && pub.externalRef) {
      ...
      this.source.cargar(
        pub.externalRef,
        filas(pub.externalRef, esc, 1 + medPrev.sincronizaciones),
      );
      ...
      pasos.push({
        nombre: 'medicion',
        estado: 'ok',
        detalle: `evaluación ${med.evaluacion?.clasificacion}`,
      });
      if (
        escenario === 'suspension' &&
        med.anomalias.some((x) => x.codigo === 'gasto_superior_autorizado')
      ) {
        incidencias.push({
          codigo: 'gasto_superior_autorizado',
          categoria: 'presupuesto',
          severidad: 'critico',
          descripcion: 'gasto por encima del límite autorizado',
          evidencia: 'anomalía de gasto',
          accionInmediata: 'pausar y reconciliar',
          estado: 'abierta',
        });
        await this.pausa.pausar(
          ctx,
          { tipo: 'departamento', valor: '*' },
          'suspensión por anomalía de gasto (ensayo)',
          'sistema',
          A,
          this.now(),
        );
        pasos.push({
          nombre: 'suspension',
          estado: 'suspendido',
          detalle: 'departamento pausado por anomalía crítica',
        });
        ...
        pasos.push({
          nombre: 'optimizacion',
          estado: 'ok',
          detalle: `${opt.decision?.tipo} (${opt.estado})`,
        });
      }
    }
    ...
    pasos.push({
      nombre: 'rollback',
      estado: 'ok',
      detalle: `rollback verificado; plan en versión ${planAntes}; métricas y auditoría conservadas`,
    });
    pasos.push({
      nombre: 'informe',
      estado: 'ok',
      detalle: 'ensayo completado en entorno no productivo',
    });
    ...
    if (resultado === 'apto_para_activacion') {
      ...
      if (e.estado === 'en_preparacion')
        await this.expSvc.transicionar(
          ctx,
          IDS_PILOTO.expediente,
          'listo_para_ensayo',
          A,
      ...
      if (e2.estado === 'listo_para_ensayo')
        await this.expSvc.transicionar(
          ctx,
          IDS_PILOTO.expediente,
          'ensayo_aprobado',
  ...
  private async finalizar(
    ...
    await this.ensSvc.registrar(
      ctx,
      ensId,
      { orgRef: IDS_PILOTO.org, escenario, pasos, incidencias, rollbackVerificado, resultado },
      A,
      this.now(),
    );
    return {
  ...
  async estado() {
    ...
    const etapasCompletas = Object.values(org.etapas).filter(
      (e) => e?.estado === 'completa',
    ).length;
    return {
      existe: org.existe,
      organizacion: org.existe
        ? {
            nombre: org.identidad?.nombreComercial,
            estado: org.estado,
            departamentos: org.departamentos,
            claseDatos: org.identidad?.claseDatos,
          }
        : null,
      onboarding: {
        total: ETAPAS.length,
        completas: etapasCompletas,
        faltantes: ETAPAS.filter(
          (e) => org.etapas[e]?.estado !== 'completa' && org.etapas[e]?.estado !== 'no_aplicable',
        ),
      },
      perfil: org.perfil
        ? {
            departamento: org.perfil.departamentoPiloto,
            modo: org.perfil.modo,
            nivelAutonomia: org.perfil.nivelAutonomia,
          }
        : null,
      politicaAceptada: org.politicaAceptadaVersion !== null,
      politicaPropuesta: org.existe
        ? { version: POLITICA_INICIAL_VERSION, propuesta: proponerPoliticaInicial(org) }
        : null,
      presupuesto: org.presupuesto ? { ...org.presupuesto } : null,
      readiness: {
        entorno: readiness.entorno,
        resultado: readiness.resultado,
        nota: readiness.nota,
        chequeos: readiness.chequeos.map((c) => ({
          codigo: c.codigo,
          estado: c.estado,
          faltante: c.faltante,
          bloqueo: c.bloqueo,
        })),
        activacionRealPermitida: readiness.activacionRealPermitida,
      },
      expediente: exp.existe
        ? {
            estado: exp.estado,
            entorno: exp.entorno,
            readiness: exp.readiness,
            intentosActivacion: exp.intentosActivacion.length,
          }
        : null,
      ultimoEnsayo: ultimo.existe
        ? {
            escenario: ultimo.escenario,
            resultado: ultimo.resultado,
            incidencias: ultimo.incidencias.length,
            rollbackVerificado: ultimo.rollbackVerificado,
          }
        : null,
      activacion: {
        bloqueada: true,
        motivo: 'la activación real es una decisión estratégica explícita pendiente',
      },
    };
  }
}
