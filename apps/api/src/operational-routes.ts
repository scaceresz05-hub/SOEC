...
export function registerOperationalRoutes(app: FastifyInstance, store: EventStore): void {
  ...
  app.post('/operativo/politicas/:id', async (req, reply) => {
    ...
    const r = await policies.registrarVersion(
      contextFrom(req),
      pid(req),
      b.contenido,
      attr(req),
  ...
  app.post('/operativo/acciones/:execId', async (req, reply) => {
    const b = req.body as {
      policyId: string;
      accion: Parameters<OperationalService['ejecutar']>[1]['accion'];
    };
    const r = await op.ejecutar(contextFrom(req), {
      executionId: eid(req),
      policyId: b.policyId,
      accion: b.accion,
      attribution: attr(req),
      occurredAt: nowIso(),
    });
    ...
    return reply
      .code(201)
      .send({
        estado: r.state.estado,
        permitida: r.decision.permitida,
        motivo: r.decision.motivo,
        efectoSimulado: r.state.efecto?.simulado ?? null,
      });
  });
