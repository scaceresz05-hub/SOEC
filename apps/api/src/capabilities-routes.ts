...
export function registerCapabilityRoutes(app: FastifyInstance, store: EventStore): void {
  ...
  const operaciones = new OperacionesService(store, ece, [
  ...
  app.get('/cap/:id', async (req, reply) =>
    reply.send({ definicion: await query.definicion(contextFrom(req), id(req)) }),
  );

  app.post('/cap/:id/ejecutar/:execId', async (req, reply) => {
    const r = await orchestrator.ejecutar(contextFrom(req), exec(req), {
      capabilityId: id(req),
      ...(req.body as object),
    } as never);
    return reply
  ...
  app.get('/cap-exec/:execId', async (req, reply) =>
    reply.send({ ejecucion: await query.ejecucion(contextFrom(req), exec(req)) }),
  );
  app.get('/cap-exec/:execId/producto', async (req, reply) =>
    reply.send({ producto: await query.producto(contextFrom(req), exec(req)) }),
  );
  app.get('/cap-exec/:execId/historico', async (req, reply) => {
    ...
    return reply.send({
      ejecucion: await query.ejecucionEnFecha(contextFrom(req), exec(req), asOf),
    });
  });
}
