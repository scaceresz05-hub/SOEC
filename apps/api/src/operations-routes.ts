...
export function registerOperationsRoutes(app: FastifyInstance, store: EventStore): void {
  ...
  const service = new OperacionesService(store, ece, [new MecanismoDeterministico(), new MecanismoSimuladoIA()]);
  const query = new OperacionesQueryService(store);
  const id = (req: FastifyRequest): string => (req.params as { id: string }).id;

  const solicitar = (operacion: TipoOperacion) => async (req: FastifyRequest, reply: import('fastify').FastifyReply) => {
    const r = await service.ejecutar(contextFrom(req), id(req), { operacion, ...(req.body as object) } as never);
  ...
  app.get('/oi/:id', async (req, reply) => reply.send({ ejecucion: await query.ejecucion(contextFrom(req), id(req)) }));
  app.get('/oi/:id/producto', async (req, reply) => reply.send({ producto: await query.producto(contextFrom(req), id(req)) }));
