...
export class ChannelExperience {
  ...
  private resumen(p: PublicationState): PublicacionResumen {
    return {
      ...
      intentos: p.intentos.map((i) => ({
        intentoId: i.intentoId,
        resultado: i.resultado,
        mensaje: i.mensaje,
      })),
      reconciliaciones: p.reconciliaciones.map((r) => ({
        tipo: r.tipo,
        resolucion: r.resolucion,
        requiereIntervencion: r.requiereIntervencion,
      })),
  ...
  async estado(): Promise<EstadoCanales> {
    ...
    for (const a of Object.values(plan.actividades).sort((x, y) =>
      x.canal.localeCompare(y.canal),
    )) {
    ...
    return {
      ...
      modo: this.modoDefecto,
      modosDisponibles: ['simulado', 'sandbox', 'real_desactivado'],
      actividades,
    };
  }
  ...
  async publicar(
    ...
    canal: string,
    modo?: ModoPublicacion,
  ): Promise<PublicacionResumen> {
    const r = await this.publicaciones.publicarCiclo(
      this.ctx(),
      this.cmd(actividadId, canal, modo ?? this.modoDefecto),
    );
  ...
  async publicarTodo(
    modo?: ModoPublicacion,
  ): Promise<{ publicadas: number; verificadas: number; bloqueadas: number }> {
    const ctx = this.ctx();
    const plan = await this.planning.cargar(ctx, IDS_MKT_CONT.plan);
    let publicadas = 0;
    let verificadas = 0;
    ...
    for (const a of Object.values(plan.actividades)) {
      ...
      const r = await this.publicaciones.publicarCiclo(
        ctx,
        this.cmd(a.id, a.canal, modo ?? this.modoDefecto),
      );
      publicadas += 1;
  ...
  async retirar(actividadId: string, canal: string): Promise<PublicacionResumen> {
    const r = await this.publicaciones.retirar(
      this.ctx(),
      `${this.paqueteId(actividadId)}__${canal}`,
      'retiro desde el centro de control',
      ATRIBUCION,
      this.now(),
    );
    return this.resumen(r);
