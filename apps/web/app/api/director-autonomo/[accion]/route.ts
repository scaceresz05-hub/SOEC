...
const POST_ACCIONES = new Set(['ejecutar-ciclo', 'pausar', 'reanudar']);
...
export async function GET(req: Request, ctx: { params: Promise<{ accion: string }> }): Promise<Response> {
  ...
  try {
    const res = await fetch(`${API_BASE}/experience/director-autonomo/${accion}${qs}`, { cache: 'no-store' });
...
export async function POST(req: Request, ctx: { params: Promise<{ accion: string }> }): Promise<Response> {
  ...
  try {
    const res = await fetch(`${API_BASE}/experience/director-autonomo/${accion}`, {
      method: 'POST',
