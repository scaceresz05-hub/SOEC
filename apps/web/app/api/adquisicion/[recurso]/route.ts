...
/** Recursos de sólo lectura del Acquisition Engine expuestos al panel. */
const GET_RECURSOS = new Set(['summary', 'channels', 'strategy', 'content', 'campaigns', 'outcomes']);

/** Reenvía la organización declarada por el navegador. Sin organización, el backend rechaza. */
...
export async function GET(req: Request, ctx: { params: Promise<{ recurso: string }> }): Promise<Response> {
  const { recurso } = await ctx.params;
  if (!GET_RECURSOS.has(recurso)) return Response.json({ error: 'NotFound' }, { status: 404 });
  try {
    const res = await fetch(`${API_BASE}/acquisition/${recurso}`, {
      method: 'GET',
