...
interface Envelope {
  status: string; objective: string; currency: string;
...
interface Resp { envelope: Envelope | null; executionAllowed: { decision: string; reasonCode: string | null }; autonomousReal: boolean; supervisedReal: boolean }
      ...
      {env && (
        <div style={{ marginTop: 10 }}>
          <ul className="s" style={{ margin: 0, paddingLeft: 18 }}>
            ...
            <li><b>Tope TOTAL autorizado:</b> {clp(env.totalCap)} <span className="muted">(máximo absoluto — no es el gasto del experimento)</span></li>
            <li><b>Presupuesto del primer experimento:</b> {clp(env.experimentBudget)}</li>
            <li><b>Máximo gasto sin contacto:</b> {clp(env.maxSpendWithoutContact)}</li>
            <li><b>Período:</b> {env.startsAt?.slice(0, 10)} → {env.expiresAt?.slice(0, 10)}</li>
            <li><b>Canal:</b> {env.plannedChannels.map(nombreCanal).join(', ') || '—'}</li>
            <li><b>Reglas de detención:</b> {env.stopRules.filter((s) => s.enabled).length} activas ({env.stopRules.filter((s) => s.enabled).map((s) => s.id.replace('STOP_', '')).join(', ')})</li>
          ...
          {env.status === 'READY_FOR_HUMAN_APPROVAL' && (
            <>
              <Callout tono="warn" ico="✍">
                Autorizo a SOEC a ejecutar acciones publicitarias dentro de este plan por un <b>máximo TOTAL de {clp(env.totalCap)}</b> durante {env.stopRules.length > 0 ? '' : ''}el período indicado, sujeto a los límites y reglas mostrados. (El primer experimento usa {clp(env.experimentBudget)}, distinto del máximo total.)
              </Callout>
