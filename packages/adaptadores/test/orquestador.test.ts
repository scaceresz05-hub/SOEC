...
const reg = (over: Partial<RegistroAdaptador> = {}): RegistroAdaptador => ({
  ...
  expiraEn: null, revocadoMotivo: null, reemplazadoPor: null, descriptor: null, nivelActivacion: 'SIMULADO', creadoPor: 'ana', actualizadoPor: 'ana-humana', existe: true, terminada: false, version: 4, ...over,
});
...
describe('@soec/adaptadores · orquestador (composición)', () => {
  it('cadena feliz: AUTORIZADO + compatible + saludable → OK + evidencia operativa', async () => {
    ...
    expect(r.evidenciaOperativa.evidenciaVersion).toBe(4);
