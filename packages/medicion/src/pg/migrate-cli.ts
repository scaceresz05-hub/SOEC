...
import { medicionMigrations } from './migrations';

/** Migra toda la cadena (… → Contenido → Canales → Medición). */
const pool = makePool();
runMigrations(pool, [...migracionesHastaCapacidades, ...operacionalMigrations, ...marketingMigrations, ...contenidoMigrations, ...canalesMigrations, ...medicionMigrations])
  .then(async (applied) => {
