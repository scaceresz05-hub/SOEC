...
import {
  medicionMigrations,
  PgMedProjectionStore,
  PgOptProjectionStore,
  drenarMedicion,
  reconstruirProyeccionesMedicion,
} from '../../src/pg';
...
describe('Medición y optimización sobre PostgreSQL real', () => {
  it('mide, evalúa, optimiza (pausa) y persiste el cambio versionado del plan', async () => {
    ...
    expect(
      (await m.planning.cargar(ctx, IDS_MKT_CONT.plan)).actividades['act-blog-0']?.estado,
    ).toBe('omitida');
  });
  ...
  it('worker: proyecta medición y optimización; reconstruye desde cero idéntico', async () => {
    ...
    const stores = {
      medicion: new PgMedProjectionStore(pool),
      optimizacion: new PgOptProjectionStore(pool),
    };
  ...
  it('concurrencia optimista, aislamiento y migración idempotente', async () => {
    ...
    await m.medicion.sincronizar(ctx, sinc(publicationId, externalRef, 'blog'));
    await expect(
      store.append(ctx, medStreamId(publicationId), 0, [
        { type: 'med.sincronizado', payload: {}, attribution: attr, occurredAt: now },
      ]),
