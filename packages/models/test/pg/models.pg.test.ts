...
import { migrations as baseMigrations, runMigrations } from '@soec/event-store/pg';
...
beforeEach(async () => {
  await ejecutarDestructivoDePrueba(
    pool,
    'truncate table events, outbox, projection_checkpoints, proj_med_current, proj_mdm_current restart identity cascade',
  );
});
...
describe('Modelos sobre PostgreSQL real', () => {
  it('MED: persiste, incorpora evidencia, revisa y relee', async () => {
    ...
    await med.registrarEntidad(ctx, {
      ...
      dimension: 'es',
      tipo: 'unidad',
      atributos: { nombre: 'Ops' },
      ...base,
    });
    await med.emitirAfirmacion(ctx, {
      instanceId: 'm1',
      afirmacionId: 'a1',
      enunciado: 'u1 ejecuta P',
      dimension: 'hace',
      incertidumbre: 'media',
      ...base,
    });
    await med.incorporarEvidencia(ctx, {
      ...
      afirmacionId: 'a1',
      relacion: 'sostiene',
      procedencia: 'bitácora',
      contenido: 'ok',
      ...base,
    });
    await med.revisarAfirmacion(ctx, {
      instanceId: 'm1',
      afirmacionId: 'a1',
  ...
  it('aislamiento organizacional y separación MED ╪ MDM', async () => {
    ...
    await mdm.registrarObservacion(a, {
      instanceId: 'x',
      observacionId: 'o1',
      contenido: 'entorno',
      ...base,
