...
describe('@soec/cia · persistencia PostgreSQL (reinicio → replay)', () => {
  it('una autorización persiste y se reconstruye tras reiniciar (nuevo store sobre el mismo PG)', async () => {
    ...
    await s1.autorizaciones.autorizar(c, CAP, { limite: 300000, nivelAutonomia: 'EJECUTAR_CON_APROBACION', actorHumano: HUMANO }, attr, O);
  ...
  it('plan aprobado, consumo, pausa y kill-switch sobreviven al reinicio', async () => {
    const c = ctx('org-pg2');
    const s1 = servicios(new PgEventStore(pool));
    await s1.autorizaciones.autorizar(c, CAP, { limite: 100000, nivelAutonomia: 'EJECUTAR_AUTOMATICO', actorHumano: HUMANO }, attr, O);
    await s1.planificador.planificar(c, 'plan-pg', { capacidadId: CAP, objetivo: 'x', costoEstimado: 4000 }, attr, O);
    ...
    const r = await s2.planificador.planificar(c, 'plan-pg-2', { capacidadId: CAP, objetivo: 'y', costoEstimado: 10 }, attr, O);
  ...
  it('aislamiento multi-tenant en PostgreSQL: una organización no ve la otra', async () => {
    ...
    await s.autorizaciones.autorizar(ctx('org-x'), CAP, { limite: 1, nivelAutonomia: 'RECOMENDAR', actorHumano: HUMANO }, attr, O);
  ...
  it('sustituir el adaptador tras el reinicio no cambia la vista de usuario (persistida)', async () => {
    const c = ctx('org-pg3');
    const s1 = servicios(new PgEventStore(pool));
    await s1.autorizaciones.autorizar(c, 'enviar-correo', { limite: 1000, nivelAutonomia: 'EJECUTAR_AUTOMATICO', actorHumano: HUMANO }, attr, O);
    await s1.planificador.planificar(c, 'pe', { capacidadId: 'enviar-correo', objetivo: 'x', costoEstimado: 100, proveedorOverride: 'correo-alfa' }, attr, O);
    const s2 = servicios(new PgEventStore(pool));
