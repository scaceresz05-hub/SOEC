...
async function ece(ctx = ctxFor('orgA')) {
  ...
  await med.emitirAfirmacion(ctx, { instanceId: 'm1', afirmacionId: 'a1', enunciado: 'x', dimension: 'hace', incertidumbre: 'media', ...cmdBase });
  await med.incorporarEvidencia(ctx, { instanceId: 'm1', evidenciaId: 's', afirmacionId: 'a1', relacion: 'sostiene', procedencia: 'A', contenido: 'c', ...cmdBase });
  await med.incorporarEvidencia(ctx, { instanceId: 'm1', evidenciaId: 'n', afirmacionId: 'a1', relacion: 'debilita', procedencia: 'B', contenido: 'c', ...cmdBase });
  await mdm.crear(ctx, { instanceId: 'w1', ambito, vigencia, ...cmdBase });
  await eceBuild.construir(ctx, { eceId: 'ece1', medInstanceId: 'm1', mdmInstanceId: 'w1', ...cmdBase });
  return ctx;
}
...
describe('Capacidades sobre PostgreSQL real', () => {
  ...
  it('concurrencia optimista sobre el stream de ejecución', async () => {
    ...
    await expect(
      store.append(ctx, capexecStreamId('x1'), 0, [{ type: 'capexec.solicitada', payload: {}, attribution: attr, occurredAt: cmdBase.occurredAt }]),
  ...
  it('versionado: la ejecución antigua no se recalcula al publicar una nueva versión', async () => {
    ...
    await orchestrator.ejecutar(ctx, 'x1', req({ objetivos: { e1: 'der:contradiccion:MED:m1:a1' } }));
