...
import {
  pilotoMigrations,
  PgEnsProjectionStore,
  PgExpProjectionStore,
  PgOrgProjectionStore,
  drenarPiloto,
  reconstruirProyeccionesPiloto,
} from '../../src/pg';
...
beforeEach(async () => {
  await ejecutarDestructivoDePrueba(
    pool,
    `truncate table events, outbox, projection_checkpoints, proj_org_current, proj_expediente_current, proj_ensayo_current restart identity cascade`,
  );
});
...
describe('Preparación de piloto sobre PostgreSQL real', () => {
  it('persiste organización + expediente; proyecta y reconstruye idéntico', async () => {
    ...
    const stores = {
      organizacion: new PgOrgProjectionStore(pool),
      expediente: new PgExpProjectionStore(pool),
      ensayo: new PgEnsProjectionStore(pool),
    };
  ...
  it('la activación real queda bloqueada y persistida; aislamiento y migración idempotente', async () => {
    ...
    // Aislamiento.
    const otra = reconstruirOrg(
      'org-1',
      'orgB',
      await store.readStream(ctxFor('orgB'), orgStreamId('org-1')),
    );
