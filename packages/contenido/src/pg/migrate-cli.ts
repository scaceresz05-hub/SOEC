...
import { contenidoMigrations } from './migrations';

/** Migra toda la cadena (… → Capacidades → Operacional → Marketing → Contenido). */
const pool = makePool();
runMigrations(pool, [...migracionesHastaCapacidades, ...operacionalMigrations, ...marketingMigrations, ...contenidoMigrations])
  .then(async (applied) => {
