...
beforeAll(async () => {
  await runMigrations(pool, [...migracionesHastaCapacidades, ...operacionalMigrations, ...marketingMigrations]);
});
afterAll(async () => {
  await pool.end();
});
...
async function sembrar(ctx = ctxFor('orgA')) {
  ...
  await planning.generarPlan(ctx, { planId: IDS_DEMO.plan, objetivoId: IDS_DEMO.objetivo, policyId: IDS_DEMO.politica, fechaInicio, opts: optsDemo, attribution: attr, occurredAt: now });
  return ctx;
}
...
describe('Marketing sobre PostgreSQL real', () => {
  ...
  it('concurrencia optimista sobre el stream del plan', async () => {
    const ctx = await sembrar();
    await expect(
      store.append(ctx, `plan:${IDS_DEMO.plan}`, 0, [{ type: 'plan.pausado', payload: {}, attribution: attr, occurredAt: now }]),
  ...
  it('worker: proyecta objetivos y planes; reconstruye desde cero idéntico', async () => {
    ...
    const stores = { objetivo: new PgObjetivoProjectionStore(pool), plan: new PgPlanProjectionStore(pool) };
  ...
  it('la migración desde base vacía es idempotente', async () => {
    const applied = await runMigrations(pool, [...migracionesHastaCapacidades, ...operacionalMigrations, ...marketingMigrations]);
