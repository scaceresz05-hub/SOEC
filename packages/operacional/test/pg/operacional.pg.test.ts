...
describe('Plano operativo sobre PostgreSQL real', () => {
  it('ejecuta y persiste una acción autorizada (efecto simulado)', async () => {
    ...
    const r = await op.ejecutar(
      ctx,
      cmd({ executionId: 'a1', policyId: 'pol-1', accion: accionOk }) as never,
    );
  ...
  it('deniega sin efecto y lo persiste', async () => {
    ...
    const r = await op.ejecutar(
      ...
      cmd({
        executionId: 'a1',
        policyId: 'pol-1',
        accion: { ...accionOk, tipo: 'enviar_masivo' },
      }) as never,
    );
  ...
  it('concurrencia optimista sobre el stream de la acción', async () => {
    ...
    await expect(
      store.append(ctx, accStreamId('a1'), 0, [
        { type: 'acc.solicitada', payload: {}, attribution: attr, occurredAt: now },
      ]),
  ...
  it('worker: proyecta políticas y acciones; reconstruye desde cero idéntico', async () => {
    ...
    const stores = {
      policy: new PgPolicyProjectionStore(pool),
      accion: new PgAccionProjectionStore(pool),
    };
