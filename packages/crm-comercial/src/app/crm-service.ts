...
export class CrmComercialService {
  ...
  /** Registra un contacto nuevo (idempotente por contactoId) y lo inscribe en el índice. */
  async registrarContacto(ctx: RequestContext, entrada: EntradaContactoNuevo, a: Attribution, occurredAt: string): Promise<void> {
    ...
    const org = this.org(ctx);
    const actual = await this.cargarContacto(ctx, contactoId(entrada));
    if (actual.existe) return; // idempotente: no re-registra
    const payload = { nombre: entrada.nombre.trim(), email: entrada.email ?? null, telefono: entrada.telefono ?? null, origen: entrada.origen };
    ...
    await this.inscribirEnIndice(ctx, entrada.contactoId, entrada.nombre.trim(), a, occurredAt);
  }

  /** Registra una actividad observable del contacto (hecho con tiempo del mundo `occurredAt`). */
  async registrarActividad(ctx: RequestContext, contactoIdArg: string, entrada: EntradaActividad, a: Attribution, occurredAt: string): Promise<void> {
    ...
    const payload = { actividadId: entrada.actividadId, tipo: entrada.tipo, detalle: entrada.detalle, valor: entrada.valor ?? null };
  ...
  /** Registra/actualiza un atributo tipado del contacto con su origen epistémico. */
  async registrarAtributo(ctx: RequestContext, contactoIdArg: string, clave: string, valor: string, origen: TipoEvidencia, a: Attribution, occurredAt: string): Promise<void> {
    ...
    const estado = await this.cargarContacto(ctx, contactoIdArg);
    if (!estado.existe) throw new ContactoNoEncontradoError(`contacto ${contactoIdArg} no encontrado`);
    const input: EventInput = { type: EVENTOS_CONTACTO.atributo, payload: { clave: clave.trim(), valor, origen }, attribution: a, occurredAt };
    await this.store.append(ctx, contactoStreamId(this.org(ctx), contactoIdArg), estado.version, [input]);
  }
  ...
  private async inscribirEnIndice(ctx: RequestContext, contactoIdArg: string, nombre: string, a: Attribution, occurredAt: string): Promise<void> {
    const org = this.org(ctx);
...
function contactoId(e: EntradaContactoNuevo): string {
  return e.contactoId;
}
