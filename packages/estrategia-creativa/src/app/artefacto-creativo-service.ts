...
export class EstrategiaCreativaArtefactoService {
  ...
  async establecer(ctx: RequestContext, estrategiaCreativaId: string, contenido: ContenidoArtefacto, a: Attribution, o: string): Promise<ArtefactoCreativoState> {
    ...
    const igual = st.existe && JSON.stringify({ ...st.artefacto, version: 0, createdAt: '', updatedAt: '', estrategiaCreativaId: '', organizationId: '', naturaleza: '' }) === JSON.stringify({ ...contenido, version: 0, createdAt: '', updatedAt: '', estrategiaCreativaId: '', organizationId: '', naturaleza: '' });
