...
## 🔨 BLOQUE F2-DISC-01 (Diagnóstico y Estrategia · vertical Clínica Dental) — EN CURSO · GATE G1 RESUELTO (2026-07-22)

Primer bloque de implementación derivado del Modelo Operativo. Directiva: `docs/decisions/f2-disc-01-directiva-diagnostico-y-estrategia.md`. Alcance: fases **Diagnóstico + Estrategia** para **un solo rubro (Clínica Dental)**; **sin efectos reales, sin IA generativa**. Único activo nuevo: el **conocimiento por rubro**; se reconecta el motor de comprensión existente (F1-UI-01).

**Gate G1 RESUELTO — ratificación parcial de la Autoridad Estratégica.** El activo de conocimiento del rubro se persiste como **biblioteca versionada y auditable** (patrón `docs/producto/rubros/README.md`; instancia `docs/producto/rubros/clinica-dental.md`), con **estados de madurez** (DRAFT→PRELIMINARY→RATIFIED→DEPRECATED), **IDs permanentes** por elemento, `confidence` (HIGH/MEDIUM/LOW) y un nuevo tipo de conocimiento **«Supuestos del rubro»**. Clasificación: **Capa Universal = RATIFIED · Capa Regulatoria = PRELIMINARY / PENDING_LEGAL_REVIEW (usable solo para advertencias y descarte conservador de estrategias, nunca para afirmar cumplimiento legal; requiere validación jurídica por jurisdicción) · Capa de Producto = RATIFIED**. Con G1 resuelto, la implementación del motor queda **desbloqueada**.

**Paso 1 — `@soec/rubros` (Biblioteca de Conocimiento por Rubro) CERRADO Y APROBADO (2026-07-22).** Paquete-biblioteca autónomo, **cero dependencias** (`node:crypto` builtin), sin persistencia ni efectos. Frontera pública **cerrada**: el conocimiento se consume solo por `RubroKnowledgePort` mediante fábricas (`crearBibliotecaClinicaDental`, `obtenerBiblioteca`); la estructura cruda, validación, canonicalización, huellas y fábrica genérica son internas. Conocimiento en tres capas (universal RATIFIED · regulatoria PRELIMINARY/PENDING_LEGAL_REVIEW · producto RATIFIED) con IDs permanentes, auditoría por entrada, **política de madurez** `SOLO_RATIFICADO`(def)/`INCLUIR_PRELIMINAR` (DRAFT/DEPRECATED nunca), **huella SHA-256** completa (64 hex) + corta (presentación), y guarda honesta `verificarCapacidadDeCertificacion`. Verificado: `tsc`/`eslint`/`prettier` limpios; **27 tests** (arquitectura, frontera pública, identidad, huella, estados, madurez, regulatorio, agnosticismo). Sin push. Aprobado por la Autoridad Estratégica con clasificación completa.

**Paso 2 — Motor de Diagnóstico — CERRADO Y APROBADO (2026-07-22).** (a) **Refactor arquitectónico obligatorio:** la definición de «Comprender el estado» se extrajo a `@soec/capacidades` como **fuente declarativa ÚNICA** (`catalogo/comprender-el-estado.ts`: `definicionComprenderEstado`, `CAPACIDAD_COMPRENDER_ESTADO_ID`); `@soec/instancia-pyme` la consume (re-export de compat en `capacidad.ts`), **sin regresión** (capacidades+instancia-pyme 50 tests verdes, incl. PG). No hay segunda definición. (b) **Nuevo paquete `@soec/diagnostico`** (capa de aplicación): reutiliza la capacidad neutral vía el motor `operaciones`/`capacidades` (no paralelo); traduce respuestas estructuradas → evidencia sintética MED/MDM → ECE → capacidad → **comprensión evaluable** con hechos/faltantes/contradicciones **con procedencia** (preguntaId→afirmacionId→evidenciaIds). Conocimiento del rubro **solo** por `RubroKnowledgePort`; **solo `MecanismoDeterministico`**; `EventStore` **inyectado** (`crearMotorDiagnostico(deps)` + `componerMotorDiagnostico(store,rubro)`), sin persistencia durable ni conexiones; una pregunta omitida → faltante en clave de ausencia de evidencia (**nunca** conclusión negativa). **No** genera estrategia, **no** registra objetivos, **no** API/UI. Verificado: `tsc`/`eslint`/`prettier` limpios; **10 tests** (determinismo, faltantes, contradicción abierta, procedencia, huella del rubro intacta, solo-determinista, agnosticismo por puerto, aislamiento). Sin efectos reales, sin push. `pnpm install` ejecutado para enlazar el paquete nuevo.

**Paso 3 — Motor de Estrategia causal — CERRADO Y APROBADO (2026-07-23).** Tras la revisión del Director, se corrigió el punto central (candidatos causalmente fundados, no solo objetivos RATIFIED disponibles). **Evolución gobernada `@soec/rubros` v1.0→v1.1** (aprobada): Capa de Producto causal — **señales `SIG-CD-*`** con `condicionActivacion` explícita (ACTIVA/INACTIVA/INDETERMINADA/CONTRADICTORIA; la mera existencia de un hecho no activa), **mapeos `MAP-CD-*`** versionados señal→objetivo→estrategia (solo señal ACTIVA habilita), **`OBJ-CD-07`** (tasa de agendamiento) + **`EST-CD-05`** (corrección de `MAP-CD-02`: baja conversión no deriva a alto valor), **activadores regulatorios tipados** por estrategia/regla; puerto `senales()`/`mapeos()`; validación referencial. **`@soec/diagnostico`** (aditivo): preserva `valor` estructurado en respuesta/hecho para evaluar señales. **`@soec/estrategia`** (contrato v1.1): `ResultadoEstrategia = PROPUESTA{candidatos, cobertura{esperados,fundados,motivoParcial?}} | ABSTENCION`; derivación causal por señal/mapeo (nunca por texto libre); un objetivo sin mapeo aplicable no aparece aunque sea RATIFIED; **regulatorio por candidato/estrategia** (activador declarado; PRELIMINARY nunca certifica; educativa sin advertencias); **`factoresConfianza`** explicables y **monotónicos**; contradicción reduce confianza, nunca la resuelve. Verificado: `tsc`/`eslint`/`prettier` limpios; **108 tests** en los 5 paquetes afectados (rubros 39 · diagnóstico 10 · estrategia 15 · capacidades/instancia-pyme 44 sin regresión). Corrección de identidad señalada: agendamiento = `OBJ-CD-07` (no `OBJ-CD-06`, ocupado por Fidelización DRAFT). Sin push.

**✅ F2-DISC-01 — ARCO COMPLETO Y CERRADO (2026-07-23):** Diagnóstico → comprensión evaluable → señales estructuradas → mapeos causales → candidatos de estrategia. Pasos 1 (conocimiento del rubro), 2 (motor de Diagnóstico) y 3 (motor de Estrategia causal) cerrados y aprobados. **`@soec/rubros` vuelve a quedar CERRADO bajo Gate G1 en v1.1.0**; cualquier cambio posterior a señales/mapeos/objetivos/estrategias/reglas exige de nuevo el procedimiento de reapertura gobernada. Un objetivo aparece solo si una regla de conocimiento explícita lo deriva desde evidencia concreta; INACTIVA/INDETERMINADA/CONTRADICTORIA no alimentan candidatos.

**Observaciones NO bloqueantes para etapas posteriores (registradas para el Paso 4/Preparación):** (1) en Preparación, distinguir **activador POTENCIAL** de una estrategia del **activador realmente SELECCIONADO** en el plan concreto (una estrategia puede admitir imágenes sin que toda ejecución las use). (2) El `valor` de una respuesta señalizada debe validarse contra el tipo esperado por la condición de la señal; una respuesta que no se pueda normalizar con seguridad debe producir **INDETERMINADA**, nunca asumir true/false.

**Paso 4 — `@soec/decision` — CERRADO Y APROBADO (2026-07-23). ✅ F2-DISC-01 COMPLETAMENTE IMPLEMENTADO Y CERRADO (pasos 1–4).** Flujo completo **Conocimiento → Comprensión → Estrategia → Decisión institucional** persistido, sin ejecutar aún ninguna acción sobre el negocio. ADR-0016 (`docs/adr/0016-decision-institucional-gobernada.md`). Límite consolidado: **Decisión ≠ Preparación ≠ Operación**. Observación registrada (no bloqueante, en ADR-0016 D-6): `ResultadoDecision` (`ACEPTADO|RECHAZADO`) es deliberadamente pequeño en v1; estados futuros (`POSPUESTO`, `NECESITA_MAS_INFORMACION`, `DERIVADO`, `EXPIRADO`) diferidos hasta necesidad real. **Siguiente bloque — F2-DISC-02 Director Workspace: DIRECTIVA DE DISEÑO APROBADA Y PERSISTIDA (2026-07-23), implementación NO iniciada.** `docs/decisions/f2-disc-02-directiva-director-workspace.md`. **Consumidor de gobierno** (motor primero, consumidor después): capa de experiencia `apps/api` (ruta estable `/experience/director-workspace/*`) que compone diagnóstico+estrategia+decisión sobre un **diagnóstico sintético sembrado** (sin cuestionario interactivo — eso es F2-DISC-03) + **página web** en el mismo checkpoint. Flujo deliberativo (Comprensión→Diagnóstico→Explicación→Alternativas→Impacto→Decisión), sección **Transparencia** (confianza global/supuestos/incertidumbre/próximos datos más valiosos), **trazabilidad expandible** (Objetivo→Mapeo→Señales→Hechos→Respuestas), acciones **Aceptar/Rechazar/Cerrar** (Cerrar = salir sin evento). Frontera dura: Workspace→DecisionService→EventStore; nunca→Preparación/Marketing. Sin Preparación/Operación/efectos/IA. Valida: «¿podría un Director gobernar solo con esta experiencia?» (si falta contexto → el defecto está en el motor). Preparación queda DESPUÉS del Workspace. **Gobernado por ADR-0017 — Divulgación Progresiva Auditable** (`docs/adr/0017-divulgacion-progresiva-auditable.md`): no ocultar complejidad por simplificación; reducir carga cognitiva por organización; toda conclusión con ruta navegable a su evidencia original; lo ausente (faltantes/contradicciones/incertidumbre/supuestos) es de primera clase. Aplica a todas las experiencias futuras. — Detalle del reforzamiento v2 del Paso 4: v2 resuelve los cuatro puntos del Director: sin resurrección histórica (estados VIGENTE/SUPERADA/REVOCADA/RECHAZADA; reemplazo explícito; RECHAZO no altera vigente; solo la vigente se revoca); concurrencia optimista (`expectedVersion`→`ConcurrencyError`) + idempotencia (`idempotencyKey`); migración gobernada `0013_objetivo_decision` (Caso A, desde base vacía, idempotente, sin borrado manual, control intacto); integridad del snapshot (`snapshotHash` SHA-256 + `verificarIntegridadSnapshot`). **27 tests.** Directiva: `docs/decisions/f2-disc-01-paso4-directiva-eleccion-gobernada.md`. Paquete **event-sourced con persistencia durable** (primero del arco): agregado por **Organización + Departamento** (`objdecision:<org>:<dep>`; a lo sumo un vigente por par); evento `DecisionRegistrada` (`ACEPTADO`/`RECHAZADO`) + `DecisionRevocada`, append-only, la última no revocada gobierna; congela la **propuesta completa** (comprensión + `ResultadoEstrategia` + candidato + huella del rubro) + **justificación estructurada** `{texto,categoria}` + actor; **autorización por rol** (`decisiones:decidir`); **decidir ≠ ejecutar** (sin adaptadores, sin efecto — prueba arquitectónica); migración `0013` con tabla propia `proj_objetivo_decision_current` (**colisión con `proj_decision_current`/stream `decision:` de `@soec/control` detectada y evitada**); proyección reconstruible. **Solo motor** (sin API/UI). Verificado: `tsc`/`eslint`/`prettier` limpios; **19 tests** (dominio 7 · servicio 8 · arquitectura 2 · PG real 2); raíz `migrate`/`start-local` → cadena `0013`. Sin efectos, sin conversión a objetivo operativo, sin push. Consumidor (revisar/elegir) = checkpoint posterior. Diseño original persistido: **Paso 4 — Elección gobernada y versionado del objetivo:** primer paquete **event-sourced con persistencia durable** del arco (`@soec/decision` — dominio de **gobierno**, no el objetivo operativo de marketing), migración `0013`. Decisiones aprobadas: agregado por **Organización + Departamento** (a lo sumo un objetivo vigente por par); evento único **`DecisionRegistrada`** con `resultado: ACEPTADO | RECHAZADO` (el rechazo también se registra); congelar la **propuesta completa** (comprensión + `ResultadoEstrategia` + candidato + huella del rubro), no solo el candidato; **justificación estructurada** `{texto, categoria: NEGOCIO|PRESUPUESTO|RIESGO|REGULATORIO|PRIORIDAD|OTRO}`; reemplazo/revocación append-only; **solo motor** (sin API/UI — el consumidor es un checkpoint posterior); **decidir ≠ ejecutar** (sin efectos, sin conversión a objetivo operativo — eso es Preparación). Nomenclatura: `departamentoId` (capacidad de negocio) ≠ `CapabilityId` cognitivo de `@soec/capacidades`. Implementación en bloque separado, tras orden. Nuevo paquete `@soec/estrategia`: **función pura y determinista** `proponerEstrategia(comprension, rubro, politica)` que consume EXCLUSIVAMENTE `ComprensionEvaluable` (@soec/diagnostico), `RubroKnowledgePort` (@soec/rubros) y una política explícita — **sin** store, sin acceso a MED/MDM/ECE ni a internals del Diagnóstico (prueba de aislamiento). Produce **candidatos, no decisiones**: objetivo/estrategia con explicación (detecté/observé/necesito/me falta), métrica, costo de medición, confianza y **procedencia** (hechos usados · faltantes relevantes · contradicciones que reducen confianza · entradas del rubro). **Madurez:** solo objetivos RATIFIED fundamentan candidatos; DRAFT/DEPRECATED nunca. **Contradicción abierta** REDUCE confianza y se conserva como procedencia — nunca se resuelve por inferencia ni se vuelve un hecho. **Regulatorio PRELIMINARY** solo advierte/bloquea conservadoramente, **nunca certifica** (declara «pendiente de validación jurídica»). **≥2 solo con fundamento**: sin hechos suficientes → **abstención evaluable** (no inventa candidatos débiles). **No** registra el objetivo, **no** persiste, **no** API/UI, **no** Preparación/Operación, sin efectos reales. Verificado: `tsc`/`eslint`/`prettier` limpios; **10 tests** (determinismo, cuello de botella→≥2 coherentes, faltantes→menor confianza, sin-hechos→abstención, contradicción sin resolución, explicabilidad+procedencia, regulatorio no certifica, agnosticismo por puerto, aislamiento). Sin push. **Debe cerrarse antes de implementar elección/versionado/persistencia del objetivo.**

## ✅ BLOQUE F2-PILOT-00 (Endurecimiento para piloto con usuarios) — IMPLEMENTADO Y VALIDADO EN VIVO (2026-07-24)

Correcciones mínimas para que 5 usuarios externos prueben el producto **sin defectos de entorno ni contaminación entre evaluaciones**, sin abrir Preparación ni el read-model de escala. Bloque único backend + frontend + validaciones + kit de Sprint 0.

- **Selección gobernada** (`apps/api/src/catalogo.ts`): catálogo cerrado de 3 organizaciones demo (Brille/Nova/Aurora) × departamento Marketing; `validarSeleccion` rechaza combinaciones inexistentes (`SeleccionInvalidaError` → 400). La UI usa `<select>`, nunca texto libre; los ids internos no se escriben a mano. Endpoint `GET /experience/catalogo`.
- **Identidad de evaluación** (`@soec/evaluacion` v2): stream `evaluacion:<org>:<dep>:<evaluacionId>` (dos sesiones NUNCA mezclan respuestas); **índice** `evalindice:<org>:<dep>` para listar; estados **BORRADOR→GENERADA→CERRADA→ARCHIVADA** (cerrada/archivada no admite cambios; nunca borrado destructivo). **Decisión AISLADA por evaluación**: el Workspace usa alcance `departamento::evaluacionId` en `DecisionService` (sin tocar `@soec/decision`), de modo que las decisiones de dos evaluaciones no se contaminan.
- **`schemaVersion` en todos los eventos** de evaluación + `interpretarVersion` (evento sin campo = v1 por compatibilidad dev; versión futura → `EsquemaEvaluacionDesconocidoError` → 422). No hubo migración destructiva.
- **Timestamps reales inyectables**: `Clock` inyectado en `buildApp`/experiencias (`systemClock` en producción, `FixedClock` en tests). El `occurredAt` del **diagnóstico** se deriva de la generación congelada (estabilidad del snapshot); los **eventos** llevan tiempo real.
- **Guardarraíl honesto**: `generacionSinEvidencia` en el DTO; la UI advierte explícitamente antes de generar sin respuestas útiles (botón «Generar de todos modos»), sin ocultar la consecuencia (probable abstención).
- **Frontend** `apps/web`: `/evaluacion` con selección + lista (reanudar/archivada) + nueva evaluación + ciclo de estados + cerrar; `/director-workspace` parametrizado por `evaluacionId` (pantalla «selecciona una evaluación» si falta). Proxy `/api/catalogo`; cliente y tipos actualizados.
- **Tres escenarios reproducibles** (`apps/api/scripts/seed-piloto.ts`, script `seed:piloto`): Caso A evidencia suficiente (2 candidatos), B incompleta (cobertura parcial 1/2, 7 faltantes), C ambigua/corregida (1 NO_NORMALIZABLE, corrección, 2 generaciones). **Reset seguro no destructivo**: re-ejecutar **archiva** las anteriores y crea nuevas (append-only). Verificado en vivo (A/B/C + reset → ARCHIVADA + nueva GENERADA).
- **Kit Sprint 0** (`docs/piloto/`): `SPRINT-0-GUIA.md` (arranque, reset, escenarios, guion de 13 tareas, preguntas posteriores, métricas de gobernabilidad/auditabilidad, señales de fracaso, checklist previo) + `HOJA-DE-REGISTRO.md` (por participante).
- **Validación viva** (navegador → proxy → API → `PgEventStore`): selección por dropdown; lista con evaluaciones sembradas (ARCHIVADA/GENERADA); **evaluación nueva** con `evaluacionId` UUID; **guardarraíl** al generar sin evidencia; captura; generación real; Workspace con candidato + trazabilidad Nivel 4 hasta la respuesta original; **Aceptar → VIGENTE** persistido (verificado por lectura fresca). Aislamiento y no-contaminación probados por test.
- **Verde**: `typecheck` (monorepo) · `lint` · `next build` · `prettier` · **suite 535 tests / 107 archivos** (9 de `@soec/evaluacion` + 15 de la API). Sin Preparación/Operación/efectos/IA. **Siguiente: Sprint 0 con usuarios reales** (lo conduce el equipo, no Claude); luego correcciones según hallazgos; después, decisión sobre Preparación.

## ✅ BLOQUE F2-DISC-03 (Captura interactiva del Director) — IMPLEMENTADO Y VALIDADO EN VIVO (2026-07-23)

Reemplaza el diagnóstico **sembrado** por la **captura real** del Director, aguas arriba del Director Workspace ya construido. Implementado en un solo bloque backend + frontend + validaciones + docs.

- **Nuevo paquete `@soec/evaluacion`** (event-sourced, **sin migración**: persiste en la tabla `events` compartida vía `EventStore` inyectado y reconstruye desde el stream `evaluacion:<org>:<dep>`, evitando cualquier cambio de esquema irreversible). Captura respuestas tipadas, **normalización SEGURA** (`normalizarBoolean`: solo valores inequívocos; lo ambiguo → `NO_NORMALIZABLE`, **nunca** asume true/false — resuelve la observación pendiente de F2-DISC-01), conserva la **entrada original exacta**, estados explícitos (`SIN_RESPONDER/RESPONDIDA/CONTRADICTORIA/NO_NORMALIZABLE`), corrección (última respuesta gobierna), borrador durable/reanudable, y **generaciones de comprensión congeladas** (`aRespuestasDiagnostico` + huella SHA-256) que alimentan el Workspace sin alterar la procedencia de decisiones ya registradas. No ejecuta Diagnóstico/Estrategia. **7 tests**.
- **Capa de experiencia** `apps/api`: `evaluacion-experience.ts` (cuestionario derivado del conocimiento gobernado del rubro —preguntas + señales—, no escrito en la UI) + rutas `/experience/evaluacion/{estado,iniciar,responder,generar}`. `director-workspace-experience.ts` **refactorizado**: ya no siembra; consume la **última generación real** de la evaluación (por `org`+`departamento`), ejecuta Diagnóstico→Estrategia sobre ese conjunto congelado (determinista) y mantiene idénticos el DTO, la trazabilidad Nivel 4 y el flujo de decisión.
- **Frontend** `apps/web`: página `/evaluacion` (selección org/departamento → cuestionario gobernado → captura con autoguardado durable → estados por pregunta → generar) + `/director-workspace` ahora parametrizado por `org`/`departamento`, con estado `sinEvaluacion` y enlace a la captura; proxies + nav.
- **Frontera §9** ampliada y verificada por test: experiencias importan solo `@soec/{contracts,event-store,rubros,evaluacion,diagnostico,estrategia,decision}`.
- **Validación viva del criterio de salida (recorrido completo en el navegador)**, contra el stack real (web 3080 → proxy → API 3081 → `PgEventStore`), con evaluación **nueva** (`clinica-sonrisa`): abrir evaluación → responder preguntas reales → **guardar y reanudar** (persistió tras recargar) → **NO_NORMALIZABLE** en vivo (Q_NOSHOW=«a veces», entrada original preservada) → **corregir** (→ «Sí») → **generar** (huella `2aeb7b13`) → Workspace con candidatos reales (cobertura 1/2 honesta) → **trazabilidad Nivel 4** hasta «Respuesta original (afirmada): Sí» → **Aceptar** → **VIGENTE** persistido (verificado por lectura fresca) → **procedencia preservada**: corregir+regenerar cambió la generación (`3c4a8aba`, candidatos OBJ-CD-01/03/04) pero la decisión ya aceptada siguió VIGENTE en OBJ-CD-01.
- **Verde**: `typecheck` (monorepo) · `lint` · `next build` · `prettier` · **suite 526 tests / 107 archivos** (7 de `@soec/evaluacion` + 8 de la API de Workspace/Evaluación). Sin Preparación/Operación/efectos/IA; decidir ≠ ejecutar; sin push. **Siguiente**: Preparación (no iniciar sin orden).

## ✅ BLOQUE F2-DISC-02 (Director Workspace — consumidor de gobierno) — IMPLEMENTADO Y VALIDADO EN VIVO (2026-07-23)

Primer **consumidor** del arco (motor primero, consumidor después). Implementado en un solo bloque: **backend + frontend + validaciones + docs**. Directiva `docs/decisions/f2-disc-02-directiva-director-workspace.md`; gobernado por **ADR-0017**.

- **Capa de experiencia** `apps/api/src/director-workspace-experience.ts` + rutas `/experience/director-workspace/{estado,decidir,revocar}` (contexto sintético server-side, permiso `decisiones:decidir`). **Compone** `@soec/{rubros,diagnostico,estrategia,decision}` sobre un **diagnóstico sintético sembrado** (clínica dental; 2 señales activas → 2 candidatos: `OBJ-CD-01` alta y `OBJ-CD-03` media con 2 advertencias regulatorias `REG-CD-01` PRELIMINARY). **Regla de no-interpretación** respetada: la experiencia no calcula confianza/explicaciones/trazabilidad — solo organiza lo que producen los motores.
- **Página web** `apps/web/app/director-workspace/page.tsx` + proxy `app/api/director-workspace/[accion]/route.ts`. Flujo deliberativo §4 (Comprensión→Transparencia→Candidatos→Impacto→Decisión), **Transparencia** (confianza global/incertidumbre/supuestos/próximos datos), **trazabilidad expandible Nivel 4** (Objetivo→Mapeo→Señal→Hecho→Respuesta original + entradas del rubro), acciones **Aceptar/Rechazar/Cerrar** + **Revocar** el vigente.
- **Frontera dura §9 verificada por test**: la experiencia importa solo `@soec/{contracts,event-store,rubros,diagnostico,estrategia,decision}`; nunca `@soec/{marketing,canales,operacional,piloto,control,…}`.
- **Validación viva (criterio §11, ambas dimensiones)**: conducida en el navegador contra el stack real (web 3080 → proxy → API 3081 → `PgEventStore`). **Gobernabilidad** ✓: Aceptar `OBJ-CD-01` → banner **VIGENTE** + persistencia durable confirmada por lectura fresca del stream; Revocar → vuelve a sin vigente. **Auditabilidad** ✓: cadena Decisión→Candidato→Estrategia→Mapeo→Señal→Hecho→Respuesta original navegable sin saltos; snapshot congelado íntegro.
- **Verde**: `typecheck` (todo el monorepo) · `lint` (además se corrigió un fallo **preexistente** de `eslint.config.js` — los scripts `.mjs` no tenían globals de Node) · `next build` · **suite 517 tests / 106 archivos** (incluye 6 nuevos de Director Workspace: frontera + estado/auditabilidad + aceptar + rechazar + revocar + 422). Sin push.
- **Alcance excluido respetado**: sin Preparación/Operación/marketing/IA/efectos reales; decidir ≠ ejecutar. **Siguiente**: F2-DISC-03 (captura interactiva) — no iniciar sin orden.

## ✅ BLOQUE F2-DOC-MO-01 (Modelo Operativo Oficial del Producto) — CERRADO Y VERIFICADO (2026-07-22)

Bloque **documental** (sin código). Tras una auditoría de producto/UX y una deliberación sobre la identidad de SOEC, la Autoridad Estratégica aprobó persistir el **Modelo Operativo de SOEC** como **documento oficial de producto**, **Opción 1**: subordinado al corpus normativo y **fuera** de la Biblioteca Maestra numerada. Archivo: `docs/producto/MODELO-OPERATIVO-SOEC.md`.

- **Jurisdicción acotada:** declara por qué existe SOEC, qué es como producto, la relación empresa–SOEC, el recorrido **Diagnóstico → Estrategia → Preparación → Operación**, los ritmos (diario/semanal/mensual) y cómo debe sentirse. **No re-legisla** soberanía humana, anti-atrofia, reversibilidad, carga de la prueba, evaluabilidad, honestidad epistémica ni criterios de éxito: **remite** a sus fuentes declarantes (Constitución Art. 2/5/7, Objetivo Supremo #2, Filosofía #3, Principios #4) — §5 del documento (Fuente Declarante Única, Art. 7.5).
- **Fisura corregida antes de escribir:** el «criterio de éxito» propuesto (éxito = trabajo que el usuario deja de hacer) **contradecía** el Criterio de Éxito declarado (#2 §5–§6). Se adoptó la forma reconciliada: SOEC asume el **trabajo de ejecución** mientras **preserva y hace crecer la comprensión, el criterio y la dirección humana**; ahorrar trabajo debilitando el criterio es dependencia, no éxito.
- **Decisiones ratificadas:** autonomía inicial **Nivel 1**; objetivo principal **siempre medible** (marca como secundaria); **tres ritmos**; **aprobador delegado diferido**. **Rubro ≠ Instancia** queda como regla de producto (aplicación de No Confusión #3 y marco/instanciación #9 inv. 12).
- **Alcance del vertical inicial:** Clínica Dental, validado contra SmileFlow como **instancia** (no como plantilla del rubro).

**Enlaces (cambios documentales mínimos):** `PROJECT_CONTEXT.md` (puntero qué/para quién/por qué → Modelo Operativo) · `README.md` (árbol de estructura + `docs/producto/`) · este `MASTER_STATUS.md` · `CHANGELOG.md`. **Validación documental:** Prettier (`--check`) sobre los archivos tocados; ESLint ignora `**/*.md` por configuración; no existe markdownlint ni verificador de enlaces en el repo. Sin cambios de código, esquema ni tests. **Siguiente hito (aún NO iniciado, por instrucción):** directiva de ejecución **F2-DISC-01 — Diagnóstico y Estrategia (vertical Clínica Dental)**; el único activo nuevo a construir es el **conocimiento por rubro**.
...
**Vertical genérica (§2, §3):** `@soec/piloto` importa solo `@soec/contracts`, `@soec/event-store` y `@soec/control` (roles); **no importa** marketing/contenido/canales/medición (prueba arquitectónica). Marketing es el primer `departamentoPiloto`, pero el contrato no se cierra a él; **no** se introduce el puerto universal «Módulo de operación» (diferido).

**El modo real es INALCANZABLE (§8, §24):** entornos sintético/emulado/sandbox/real_desactivado/real_preparado/**real_habilitado**; `real_habilitado` está contemplado pero **imposible de alcanzar** (`realHabilitable()` → false siempre); ninguna organización llega a `activa` en modo real (máximo `lista_para_activación`).

**Onboarding versionado y reanudable (§6):** agregado `org:<id>` con etapas (identidad…revisión), cada una con estado/datos/faltantes/responsable; distingue dato real/sintético/pendiente/no_aplicable; la ausencia queda visible y **no se inventa**. Perfil operacional, presupuesto y conexiones previstas (sin tokens) sobre el mismo agregado. **Política inicial** propuesta de forma conservadora (escalamiento con aprobación, autonomía ≤ 3), aceptada explícitamente y versionada.

**Readiness DETERMINISTA por entorno (§12–§14):** chequeos estructurados (código/estado/severidad/faltante/bloqueo); distingue **pendiente** de **bloqueado**; una **credencial fixture** de sandbox **no basta** para un entorno real; un activo-especificación no basta para un canal visual real. La **activación real nunca se aprueba** (`activacionRealPermitida: false` siempre). **Presupuesto** con categorías separadas y `ejecutadoReal` del tipo literal `0` (garantizado por tipo).

**Ensayo integral, rollback y expediente (§15–§20):** el ensayo recorre onboarding→readiness→política→plan→contenido→publicación EMULADA→métricas→optimización→pausa→**rollback**→reanudación→informe; cada ejecución es un ensayo distinto, idempotente. Escenarios A–H. **Expediente** (`exp:<id>`) versiona alcance/criterios de éxito/suspensión/rollback/readiness/checklist; **nunca** alcanza `autorizado` real. **Ceremonia de activación SIEMPRE BLOQUEADA:** `intentarActivacion` deniega, registra el intento y devuelve las autorizaciones faltantes (API 409); no existe endpoint para activar real, gastar, generar token productivo ni saltar la autorización.

**Resultados exactos (2026-07-21):** `pnpm -r typecheck` OK (22 workspaces, incl. `@soec/piloto`) · `pnpm lint` limpio · backend `pnpm test` **425 passed (82 files)** — nuevos: piloto dominio 8 · arquitectura 3 · pg 2 (Postgres real) · api 3 = **16** · web `pnpm -C apps/web test` **6 passed** · `next build` ✓. Migración desde **base recién creada** `0001…0012` (nuevo `0012_piloto_projection`); contenedores `ssr_*` intactos.

**Validación visual real (app viva, cadena real en PostgreSQL):** `/piloto` — organización sintética **lista_para_ensayo**, onboarding **17/17**, perfil marketing/sandbox/autonomía 3, presupuesto **real ejecutado 0**, readiness **apto_para_ensayo (sandbox)** con todos los chequeos aprobados, y banner **«Activación real: BLOQUEADA»**. En vivo (base limpia): ensayo **exitoso → apto_para_activacion** (rollback verificado); **suspension → suspendido** (1 incidencia crítica, pausa + rollback); **onboarding_incompleto → bloqueado**; **intento de activación → HTTP 409** con las autorizaciones estratégicas faltantes. Casos A–H probados en la suite. (Capturas raster siguen excediendo el timeout del entorno; validación por texto de la app viva, no PNG.)

**Siguiente nodo:** ya **no es otro componente sintético**, sino la **decisión estratégica** de un **piloto real** (empresa/marca/objetivo/canal/cuenta/contenido/presupuesto/modo/nivel/aprobación/pausa/duración/indicadores/criterios de éxito y suspensión). Hasta esa autorización, **todo efecto externo real continúa prohibido**.

**Deuda técnica / límites:** organización y proveedores **sintéticos/emulados** (sin datos/credenciales/gasto reales); atribución/administración de usuarios y programa legal de protección de datos como mínimos (no completos); el puerto universal «Módulo de operación» sigue diferido; el ensayo reutiliza el pipeline de marketing bajo la experiencia (los conceptos del piloto son genéricos, la orquestación del ciclo consume contratos de marketing).

## ✅ BLOQUE F2-CTRL-HARD-01 (Endurecimiento Genérico Previo al Piloto) — CERRADO Y VERIFICADO (2026-07-21)

Bloque **corto y acotado** tras la revisión arquitectónica: se aprobó la **Opción A** (endurecimiento mínimo), rechazando dejar las fricciones (B) y el puerto universal especulativo (C). Regla: *preparar los puntos de extensión evidentes; extraer la abstracción completa solo con dos implementaciones reales.* Solo elimina las restricciones superficiales que impedían que futuros departamentos incorporen tipos sin tocar `@soec/control`. **No** crea el puerto «Módulo de operación», ni resumen genérico, ni adaptadores ficticios. Complemento de ADR-0014.

- **Catálogos extensibles:** `TipoDecision` y `TipoAlerta` pasan de uniones cerradas a catálogos abiertos (`string`) con validación de **formato** (no membresía; rechaza vacíos/malformados) y **catálogo base documentado** separado de las primitivas. Los **estados** siguen siendo uniones **cerradas**. Un departamento futuro (ventas: `aprobar_reembolso`; inventario: `quiebre_stock`) registra sus tipos sin modificar el núcleo. **Sin migración de datos.**
- **Pausa por alcance genérico:** el chequeo opera sobre `Alcance {tipo, valor}` por igualdad + precedencia (global precede), sin condicionales que nombren `canal`/`campania`; el llamador aporta la cadena de ancestros. Un alcance futuro (`inventario:almacen-1`) funciona sin tocar el agregado; eventos ya persistidos compatibles.

**Resultados (2026-07-21):** `pnpm -r typecheck` OK (20 workspaces) · `pnpm lint` limpio · backend **409 passed (78 files)** (nuevas: 2 de endurecimiento — alcance futuro + catálogos extensibles/malformados rechazados; marketing sin regresión) · web **6 passed** · `next build` ✓. Sin cambios de esquema (migración `0001…0011` intacta). Compatibilidad total: API, interfaz `/control`, pausa maestra, decisiones, alertas y reconstrucción sin romperse. **Diferido explícitamente:** el puerto universal «Módulo de operación» hasta un segundo departamento real. Siguiente: **F2-PILOT-01** (piloto real acotado; conceptos de empresa/onboarding/entornos/readiness/monitoreo/rollback nacen genéricos bajo «organización/operación»). Efecto/gasto/credenciales reales siguen prohibidos.

## ✅ BLOQUE F2-CTRL-01 (Centro de Control del Departamento Autónomo) — CERRADO Y VERIFICADO (2026-07-21)

Sexta vertical del Departamento de Marketing Autónomo: la experiencia de **dirección, gobierno y control** que reúne las cinco verticales previas. El propietario comprende, autoriza, limita, pausa y audita al departamento **sin operarlo cotidianamente**. Paquete `@soec/control` + experiencia `/control`; ADR-0014. Modo real desactivado; ningún efecto/gasto real.

**El propietario dirige; SOEC realiza el trabajo (§2):** la experiencia prioriza estado, excepciones, presupuesto, resultados, riesgos, próximas operaciones, decisiones pendientes y trazabilidad; no editores manuales ni microtareas; lenguaje de **trabajo realizado**, no consultivo.

**Modelo de lectura integrado, no otra fuente de verdad (§3):** el resumen se **compone en la capa de aplicación** a partir de **contratos públicos y proyecciones** de marketing/contenido/canales/medición/operacional/control. `@soec/control` **no importa los dominios operativos** (prueba arquitectónica): solo aporta sus agregados (pausa, decisiones, buzón) y los tipos del modelo de lectura; no recalcula indicadores, no reinterpreta estados, no muta agregados ajenos.

**Salud determinista por precedencia (§16):** saludable / advertencias / degradado / parcialmente_bloqueado / intervencion_requerida / pausado / sin_informacion, derivada de señales explícitas (1 pausa total · 2 riesgo crítico · 3 intervención · 4 bloqueo · 5 advertencias · 6 saludable · 7 sin datos), no de narrativa libre.

**Interruptor maestro de pausa, real e integrado (§9):** pausa por departamento/canal/campaña/tipo de acción, event-sourced con propagación; **cuando hay pausa el ciclo no produce nuevos efectos** (publicar/optimizar/escalar) pero las lecturas, verificación y auditoría continúan. No es visual: el ciclo consulta la pausa antes de cada efecto (verificado en vivo: `simular` pausado → 0 efectos).

**Bandeja de decisiones con permisos por rol (§11, §20):** decisiones excepcionales (escalamiento, presupuesto, canal, modo…) registradas como pendientes y resueltas (aprobar/denegar/modificar/posponer) con permisos por rol (propietario/supervisor/observador/operador técnico); alto riesgo exige `aprobar_alto_riesgo`; al aprobar un escalamiento el efecto se aplica como **cambio versionado del plan** (contrato público). No hay aprobación genérica sin alcance; una decisión resuelta no se re-resuelve. **Alertas** deduplicadas por clave con severidad/evidencia/acción/estado; **notificaciones** internas (sin correos/push reales) que referencian la alerta/decisión; **auditoría** que reconstruye la cadena objetivo → plan → publicación → medición → optimización sin exponer secretos. **Aislamiento multiempresa.**

**Resultados exactos (2026-07-21):** `pnpm -r typecheck` OK (20 workspaces, incl. `@soec/control`) · `pnpm lint` limpio · backend `pnpm test` **407 passed (78 files)** — nuevos: control dominio 6 · arquitectura 3 · pg 2 (Postgres real) · api 3 = **14** · web `pnpm -C apps/web test` **6 passed** · `next build` ✓. Migración desde **base recién creada** `0001…0011` (nuevo `0011_control_projection`); contenedores `ssr_*` intactos.

**Validación visual real (app viva, cadena real en PostgreSQL):** `/control` como entrada principal. Estado inicial «Parcialmente bloqueado», modo **simulado** («real desactivado · sin gasto real»), autonomía nivel 3, trabajo (3 piezas, 3 publicaciones verificadas, 6 campañas), actividad. Tras un ciclo: **optimizaciones aplicadas 3**, presupuesto consolidado con **discrepancia 300** (gasto > planificado), objetivos `bajo_umbral` (evidencia alta, 0.0% vs meta 5.0%), línea de actividad en lenguaje de trabajo («SOEC pausó la actividad… el plan quedó versionado»). **Pausa maestra en vivo:** botón → estado **«Pausado»**; `simular` estando pausado → **0 efectos** (pausa operacional real, no visual); botón → «Reanudar departamento». Escenarios de decisión (aprobar→efecto versionado), alerta de gasto y auditoría probados en la suite de integración. (Capturas raster siguen excediendo el timeout del entorno; validación por texto de la app viva, no PNG.)

**Siguiente nodo habilitado:** **F2-PILOT-01** (piloto real acotado). Requerirá una **decisión estratégica expresa** (empresa/marca/objetivo/canal/cuenta/contenido/presupuesto/modo/nivel/aprobación/pausa/duración/indicadores/criterios de éxito y suspensión). Hasta entonces, todo efecto externo real continúa **prohibido**.

**Deuda técnica / límites:** datos y proveedores **sintéticos/emulados** (sin operación real, sin gasto real); roles/permisos como matriz mínima (sin administración completa de usuarios ni autenticación real); actualización en tiempo casi real por **polling** controlado (no SSE/WebSocket); la pausa se integra vía la orquestación de la experiencia de control (los endpoints operativos directos de otros módulos conservan su propio comportamiento); auditoría de cadena por canal (no un explorador universal de grafo).
